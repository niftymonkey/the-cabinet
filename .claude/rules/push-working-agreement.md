...
- Before every commit that contains code, run the CodeRabbit CLI (`coderabbit review --agent --base main -t committed`, or with `--api-key` where no login exists) and address its findings. Run the relevant review gate agents through the commits. Never commit unverified code; fix warnings and build errors first.
