# Slice 7: the Undertaker's end (design record R6, #106)
...
- A finding rides with this slice, because step 4 below works in the same function. `levelsFromUrl` (`src/app/seedFromUrl.ts`) accepts "zero to the max line level", and `checkLevels` (`src/game/invariants.ts`) gives every birthright line a floor of 1, so `?levels=0` opens on THE GAME BROKE with `FAULT levels in range AT TICK 1` (slice 3's coder met it). A URL is a live environment input, so it is repaired to a safe value and the repair is logged: a value below 1 goes through `ignoreLevels` exactly as a value above `MAX_LEVEL` does, and the JSDoc's "zero to the max line level" becomes "one to the max line level". One new test in the file that already tests `levelsFromUrl`: "a pinned level of zero is ignored, because a birthright line never stands below one". If any caller or test relies on `?levels=0`, stop and report.
- A second finding rides with this slice, because it is the replay screen's and this branch made it reachable. `primePlayback` (`src/app/screens/tapePlaybackSession.ts`) speaks a refusal for `rosterNotImplemented` and for `witnessVersionMismatch` only. A tape refused as `conditionNotImplemented` (any tape recorded before slice 1, which does not name `swallow.tipThreshold`) falls through to a bound of zero, and the screen reads `PLAYED TO TICK 0, THE LAST VERIFIED CHECKPOINT` over a drawn starting field with no reason given, while `scripts/measure.ts` over the same bytes names the missing row. Nothing abnormal is ever silent: give `conditionNotImplemented` a spoken refusal in the form the other two have, carrying the reason the playback result already holds. One new test beside the existing refusal tests in `src/app/screens/__tests__/tapePlaybackSession.test.ts`: "a tape that does not name a starting condition this build requires is refused by the name of the condition". A tape to check it by hand is any under `local/148-before-batch/batches/`. If the playback result does not carry the reason, stop and report.
- A lesson from slice 5 for any replay test you write: a replay rebuilds the run from the seed and the commands alone, so a run staged by writing to its state cannot be replayed. Play to the state, or use the won tape.
...
Every planned test is green, every verification step has a result in your note, the proof tape verifies, no pin moved, the two comments and the one sentence in `game-concept.md` tell the ending as built, and the working tree holds the code, the tests and `docs/branch/records/slice-7-note.md`, uncommitted.
