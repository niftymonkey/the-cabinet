...
- Last landed slice: slice 4, the fall and the teeter. Its note is `docs/branch/records/slice-4-note.md` and its screenshots are under `local/148-slice-4/screenshots/`. The `swallowed` event now carries the food's place, way, size and look, `fall.ts` is the pure fall that slice 6's Undertaker reuses, and the proof tape still verifies. Before it, slice 3, the grave drawn as a hole cut in the ground. Its note is `docs/branch/records/slice-3-note.md`, its screenshots are under `local/148-slice-3/screenshots/`, and the proof tape still verifies. It created `graveProjection.ts`, `graveDrawingValues.ts` and the `falls` container that slice 4 fills, and it paid #121's assertion in `layering.ts`. Before it, slice 2, the pull: `docs/branch/records/slice-2-note.md`, with its figures in `docs/branch/records/after-slice-2-batch.md`, which holds the one table where the first-touch build, slice 1 and slice 2 can be read side by side. Slice 1 (the new swallow rule) landed before it: `slice-1-note.md`, `after-slice-1-batch.md` and the last section of `before-batch.md`. `WITNESS_VERSION` is 12 now and `GOLDEN`'s checksum was re-pinned, as slice 2's entry ordered.
...
- The next work, in order: check usage (`~/.claude/skills/stay-within-limits/usage.sh`), then dispatch slice 5's coder (Opus), the replay that plays to the run's true end. Slices 1 to 4 are deployed at `hungry-grave.vercel.app`. A deploy builds from a clean detached checkout, never from this worktree while a coder has uncommitted work in it: `git worktree add --detach <scratch> <commit>`, copy `apps/hungry-grave/.vercel` from the main repo folder, `pnpm install --frozen-lockfile --prefer-offline`, then the recipe in `apps/hungry-grave/docs/deploy.md`, then `vercel ls` and a `curl` of the alias to confirm. The dispatch points the coder at its entry file, the coder contract and `docs/agents/feature-flow.md`, and need not paste the entry, because the dispatch hook checks only for the feature flow's path.
...
- The rim hides the hole at the small sizes, and this bears on #148's own finish line. The rim strokes 4 field units inward on each side at every size (ADR 0003 puts its outer edge on the hitbox, and its width is bracketed from a phone's pixels for ADR 0014). The side walls are 16.3% of the opening, so at the ceiling size both walls read, at the start size (27) about one pixel of wall survives, and at the floor (18) none does. At real phone scale the start-size grave is a small pale outlined box. The prototype he approved had no rim, which is why its wall shares were four times these. The slice is built as R4 rules. Pictures: `local/148-slice-3/screenshots/size27-bottom.png`, `zoom-27-low.png`, `zoom-675-low.png`, `size33-zoom.png`. What the rim does for him: it shows the exact mouth the 55% rule measures, it carries the reservoir's glow, and it carries Territory's arc.
