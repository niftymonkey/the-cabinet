...
- Last landed slice: slice 6, the prototype's grave drawing ported line for line (`docs/branch/records/slice-6-note.md`, side-by-sides under `local/148-slice-6/screenshots/`). The ground is unchanged, so the lip bites and trodden margin nearly vanish on the dark tile. Deployed to hungry-grave.vercel.app on 2026-09-21 and Mark was told. Next: the ground and colour talk once he has looked, then slice 7. Before it, slice 5, a replay plays to the run's true end. Its note is `docs/branch/records/slice-5-note.md`, with a Landing section. The won tape for slice 7 (the Undertaker's end) is `local/148-won-tape/3000.tape` (victory at tick 23,470; all 393 checkpoints verify). Before it, slice 4, the fall and the teeter (`slice-4-note.md`, screenshots under `local/148-slice-4/screenshots/`): the `swallowed` event carries the food's place, way, size and look, and `fall.ts` is the pure fall the Undertaker's end reuses. Slice 3 drew the grave as a hole (`slice-3-note.md`); it created `graveProjection.ts`, `graveDrawingValues.ts` and the `falls` container. Slice 2 is the pull (`slice-2-note.md`, figures in `after-slice-2-batch.md`). Slice 1 is the new swallow rule (`slice-1-note.md`, `after-slice-1-batch.md`, the last section of `before-batch.md`). `WITNESS_VERSION` is 12.
...
- The next work, in order. (1) The ground: Mark's pick between the prototype's ground on the whole field, a patch round the grave, or the dark ground kept. (2) Slice 7, the Undertaker's end (entry `docs/branch/records/slice-7.md`, needs nothing from Mark). (3) Correct R4 in the design record (it still describes slice 3's rim and calls the Undertaker slice 6). (4) The three gates on the built result, then fold and delete the working records. (5) The branch close by `end-of-the-branch.md`.
