...
- Last landed slice: slice 3, the grave drawn as a hole cut in the ground. Its note is `docs/branch/records/slice-3-note.md`, its screenshots are under `local/148-slice-3/screenshots/`, and the proof tape still verifies. It created `graveProjection.ts`, `graveDrawingValues.ts` and the `falls` container that slice 4 fills, and it paid #121's assertion in `layering.ts`. Before it, slice 2, the pull: `docs/branch/records/slice-2-note.md`, with its figures in `docs/branch/records/after-slice-2-batch.md`, which holds the one table where the first-touch build, slice 1 and slice 2 can be read side by side. Slice 1 (the new swallow rule) landed before it: `slice-1-note.md`, `after-slice-1-batch.md` and the last section of `before-batch.md`. `WITNESS_VERSION` is 12 now and `GOLDEN`'s checksum was re-pinned, as slice 2's entry ordered.
...
- The next work, in order: check usage (`~/.claude/skills/stay-within-limits/usage.sh`), then dispatch slice 4's coder (Opus), the fall and the teeter. Its prompt tells the coder to load the `pixijs-skills:pixijs` skill. After slice 4 lands, deploy to `hungry-grave.vercel.app` (pre-authorized, recipe `apps/hungry-grave/docs/deploy.md`), so Mark can play the rule, the pull, the hole and the fall together. The dispatch points the coder at its entry file, the coder contract and `docs/agents/feature-flow.md`, and need not paste the entry, because the dispatch hook checks only for the feature flow's path.
...
See the list at the end of the design record. From slice 1: how the 55% feels on real corpses under fire, and whether a very fast swipe under a corpse ever misses. From slice 3: whether the grave reads as a grave in the ground at the start size, and the colour of the tufts (a dark grey-green, `graveTurf` `#263121`, parted from the ground by hue alone). From slice 2: whether the pull feels gentle and right under fire, and a slip between two power-up options to see that neither moves. Nothing is deployed yet; the first deploy worth his time is after slice 4, when the rule, the pull, the hole and the fall can all be seen together.
