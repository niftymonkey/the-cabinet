# The cabinet composes games by reference, never by import

The cabinet may present a game in whatever way suits that game: navigating to it, opening it in a new window, offering a download, or embedding it on the cabinet's monitor so the player appears to lean in and the game takes over the viewport. All of these stay open, and which one a given game uses is a per-game choice.

What is ruled out is the cabinet importing a game's code. The cabinet never imports a game module, never mounts a game onto its own PixiJS stage, never shares a ticker or an asset loader with one, and never holds game state. A game is referenced by URL or by artifact, and that is the whole contract.

This is what keeps a game a self-contained application with its own technology stack, per ADR 0001. Mounting a game into the cabinet's own renderer would force a shared PixiJS version, a shared frame loop and shared asset paths on every game in the cabinet, which is the coupling that decision exists to prevent.

## How embedding stays within the rule

An `<iframe>` satisfies the contract, because the game inside it is still an independently built application reached by URL. PixiJS `DOMContainer` drives an element's CSS transform from the scene graph, so an iframe positioned on the cabinet's monitor follows the lean-in, scale and opacity of the cabinet itself. That gives the immersive route without any coupling, and because it is an iframe it works for plain HTML games exactly as well as for PixiJS ones.
