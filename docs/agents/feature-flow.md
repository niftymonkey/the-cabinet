...
- The test names compared before and after the slice, never the pass counts, so a removed or renamed test shows up. The floor, always present. The report explains every name that was lost. How to capture and compare: `docs/agents/lessons.md`, "Compare test names before and after". Actor: the agent.
- `pnpm typecheck` and the production build. Editor diagnostics are not a judge; these are. Actor: the agent.
- Player-visible change: a rendered check of the built app (`vite preview`, never only the dev server), with the screenshot actually read. A screenshot covers a state that lasts. For a state that is over in a few frames (a flash, a tell, a fall, a shove), the agent stages a scenario that holds the state, or steps it tick by tick, and photographs it there. Where the harness cannot hold the state, building that ability is part of the slice and is named in step 2. The human's play never stands in for this check. Actor: the agent.
...
- One suspected weak test: a mutation spot check. Copy the code to a scratch folder, break the line the test guards, and run that one test against the copy. A test that stays green is weak. The mutation lives and dies in the copy; the working tree is never mutated and never restored from version control. Actor: the agent.

A check earns its place by failing only when its promise breaks. A check that fails on every change, or on a harmless one (a retitled test, a moved number), gets steered around: the work bends to avoid it, and then it guards nothing. An agent that meets one reports it as a finding, and the fix goes to the check, never to the work.
