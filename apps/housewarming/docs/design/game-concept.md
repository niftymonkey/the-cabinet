...
**Deduction.** A positive result says several things at once: it came, so the room was right or it wanders, the lure matched, the hour overlapped, and whatever was used as a ward is *not* its aversion. A negative result is the interesting one because it is ambiguous: wrong room, wrong lure, wrong hour, or it was warded off by accident. That ambiguity is the puzzle, and it is what makes a careful experiment better than a random one.
