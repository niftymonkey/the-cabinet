...
4. **Update the book.** Strike traits out and circle them in.
...
Start at four values per axis, 256 names. The house growing widens the space, but the space cannot grow far. Of the three openings, the first is spent on rooms (four to seven), the second changes what an instrument can say and widens nothing, and the third widens lure to six; the hour axis cannot widen at all, because a daytime hour has no instrument to test it. So the space tops out near 4 by 6 by 4 by 7, roughly 670 names, not Mastermind's 1296. The ramp is not the search space. It is the economy tightening as the season turns, arrivals staggered so late spirits land mid-solve, and parallel loose spirits sharing the keeper's nights, with modest pool growth on top. The kernel already measured the economy as the difficulty dial (see the decision log, 2026-08-08).
...
**Leaving is wintering out.** Taking Leave does not give the house up. The keeper locks the door from the outside, pockets the key, takes the book, and comes back when it thaws. The run after a leave is the return: the same house, another autumn, fresh spirits stirring, and a flat head start of warmth banked as provisions, the same amount every time and never stacking. The offer screen says both halves plainly, stay and risk the window closing, or leave and next winter starts warmer, so the threshold gamble is two concrete things priced in the same currency. Freezing loses both halves: the house passes over your body to the next keeper, and you join the drain you were fighting. Inheritance needs a death, which is how her book reached you.

The return is acknowledged with one image and no text: the run opens on the door being unlocked, provisions in, the higher warmth visible. The intro stays as settled. The head start lives in one profile flag beside the versioned save, and the profile accretes nothing further without a fresh decision.
...
- **The loss condition is freezing.** If the drain wins past the window, the keeper dies of cold the way she did and the house passes to the next keeper. Leaving before the window closes is a retreat rather than a loss: see the wintering out above.
...
So the house does the pacing. It opens up as you name: the sealed wing, her finer tools, the pantry. Each opening changes exactly one thing about the puzzle, one axis's pool or what one instrument can say, and each is a place or thing in the house, never a menu unlock. That is the unfolding pillar, and it fits a single house that you love and are slowly taking back.

Settled by ticket #6 (see the decision log, 2026-08-09). Revisit if the middle of the game goes flat.
...
- **2026-08-06** The house opens a small known number of times, two or three, triggered by naming rather than by a date, each opening widening exactly one axis. Rewritten 2026-08-09: three openings, and each changes exactly one thing about the puzzle, one axis's pool or what one instrument can say; see the ticket #6 entries below.
...
- **2026-08-09** Leaving is wintering out, not surrender. A keeper who takes Leave keeps their life, the house, the key, and the book, locks the door from the outside, and comes back when it thaws. Freezing is the only way the house passes to a next keeper, which is how her book reached the player, because inheritance needs a death. What freezing uniquely loses is the keeper: the line advances over your body. The book alone could never distinguish the endings, since the book survives freezing too, which is the premise. From ticket #23.
- **2026-08-09** The run after a leave is the return: the same house, another autumn, and the spirits stirring that year are not last year's, which the roster-is-not-a-census decision already carries, so the reroll is diegetic without touching the rejected house-after-house shape. Old names count for nothing because a house left unkept all winter comes loose again. From ticket #23.
- **2026-08-09** Leaving banks a flat warmth head start for the return, the same amount every time and never stacking, so departures cannot be farmed. Provisions in the fiction, never carried progress. The number is #15's to measure, with the post-leave run a first-class solver case since leaving is the rational way to reroll a bad roster, under two falsifiable bounds: tanking a run to bank the head start must never beat playing on, and the warm start must not trivialize the opening nights. From ticket #23 and its gate reviews.
- **2026-08-09** The Leave offer states both halves plainly: stay and risk the window closing, or leave and next winter starts warmer. A dilemma the player cannot price is not a dilemma. The offer's visibility window remains #15's structural rule. From ticket #23.
- **2026-08-09** The return is acknowledged with one image and no text changes: the run opens on the door being unlocked, provisions in, the higher warmth visible, and the intro stays exactly as #5 settled it. The head start lives in one profile flag beside the versioned save, and the profile accretes nothing further without a fresh decision, so year two inherits no meta-progression scaffold. Reaffirms no-prestige and no-roguelite-reset: a one-shot non-stacking head start gated behind a failing run stacks nothing, grinds nothing, and farms nothing. From ticket #23.
- **2026-08-09** The three ending images are mirrors: leave is the keeper outside the locked door with the key pocketed and the book under their arm, freeze is the intro's found-frozen image except it is you, and the return is the same door unlocked again. Routed to #17 and #20 with the offer screen. From ticket #23.
- **2026-08-09** The roster is six, settled ahead of everything else on the openings ticket because every other number there is a function of it. The kernel is built and measured at six, six distinct haunts force at least seven rooms under the night contract, and the supply of clean attested trace kinds caps the roster near seven or eight from the other side. From ticket #6.
- **2026-08-09** The house opens three times, giving four stages. Two beats over a thirty-to-fifty-night run leaves long stretches where the house is done surprising you, the first opening is spoken for by arithmetic, and three is what leaves room for one instrument change and one pool widening. Settles the "two or three" of 2026-08-06. From ticket #6.
- **2026-08-09** Each opening changes exactly one thing about the puzzle: one axis's pool, or what one instrument can say. This loosens the 2026-08-06 "exactly one axis" rule by the smallest amount, the one the kernel comment on #6 predicted; the load-bearing part was always one legible change per beat, not axis-hood. Every opening is also a place or thing in the house, because the pillar is "the house does the pacing" and a widening that surfaces as a menu unlock would quietly replace it. From ticket #6 and its gate reviews.
- **2026-08-09** Opening 1 is rooms, four to seven in a single beat, fictionally one sealed wing rather than three separate doors. Three door beats would spend the whole opening budget on one axis, and one clean haunt-pool jump changes which inferences are possible once, properly. Spirits draw haunts from the pools in force at their arrival stage, so every pre-wing spirit is known to live in the original four rooms and the doubling swamps nobody mid-solve. The "cellar, attic, locked room" line was always proposed, not settled. From ticket #6 and its gate reviews.
- **2026-08-09** Opening 2 changes what one instrument can tell the keeper and widens no pool. It lands mid-game where dead mornings threaten, so richer readings arrive when the juggling is heaviest, and fictionally it is hers: her finer tools or a page of her method, the one carrier of the aunt thread between the intro and the coda. The concrete change is deliberately an undesigned slot with its own ticket, ordered before #14 pours stage values, its cheapest test candidate changes run headlessly through the kernel, and its named fallback a pool widening, so the slice never stalls on it. From ticket #6 and its gate reviews.
- **2026-08-09** Opening 3 widens lure from four to six. Only stage-four arrivals draw from the wider pool, so the widening lands on the final one or two solves, worked with the better instrument already in hand against the worst of the winter drain: a final exam, not a second ramp. Fictionally a found space that is not a haunt, the pantry or the garden. That at least one arrival, ideally two, lands after opening 3 is a generator invariant the solver verifies per house, with the same standing as rooms outnumbering the roster and she-is-findable, because otherwise the pool widens and the play does not. From ticket #6 and its gate reviews.
- **2026-08-09** An opening fires the morning after the Nth naming, a cumulative count, the Ns being #14's to measure. DONT-BUILD the warmth-threshold trigger: it couples the content ramp to the dial the kernel measured as difficulty, so a struggling keeper gets a smaller, staler house while losing, the anti-spiral rule again. DONT-BUILD the specific-spirit trigger: it fights the keeper's freedom to choose the order names fall in, and per-run rosters make it an accident. The count keeps all randomness at roll time, so no RNG state ever enters the save. Delaying a naming to finish a solve is a real plan the trigger allows, so #14 and #15 carry the bound that holding a ready name never beats naming and opening. How the count is disclosed is #24's to design in the book's voice, never a meter. From ticket #6 and its gate reviews.
- **2026-08-09** An opening never rewrites evidence already read and never widens a puzzle in progress: it can only add rooms, add pool values for spirits not yet arrived, or add to what future mornings can say. The instrument opening is the one-directional exception, enriching future mornings including about live puzzles, while an old morning keeps exactly the meaning it had when read. Arrival stage thereby stays evidence, and the 2026-08-08 arrival-stage-pools decision extends cleanly. The book must show the narrower pools on an old spirit's page, the new rooms simply absent, or a keeper wastes nights probing a pantry lure on a spirit older than the pantry; routed to #24. From ticket #6 and its gate reviews.
- **2026-08-09** Hour widening is DONT-BUILD: the only honest values leave the night, the candle cannot test a daytime hour, and a day instrument would be a new invention, not one more list entry. Revisit only if a day instrument is ever designed on purpose. Aversion widening is rejected pending measurement rather than on taste: the lure is in every experiment so the bowl probes it every night for free, while the ward is optional and a held ward blinds the bowl, and #12 and #13 verify that probe arithmetic against the resolver before #14 bakes lure four to six. From ticket #6 and its gate reviews.
- **2026-08-09** The keeper's hand on a page does two things, strike and circle: any pool value can be struck out or circled in, at most one circle per axis, and four circles fill the entry's name line ready to submit. Neither is ever verified by the game, so both can be wrong, which is Strange Horticulture's unverified labels rather than Obra Dinn's auto-locking book, a deliberate divergence because verification already lives in priced whole-name submission. Strikes and circles are plain data in the versioned run save, so a restored run restores the keeper's thinking. The word mark stays the candle's alone; the book's acts are strike and circle everywhere, interface and code. From ticket #24.
- **2026-08-09** The book keeps a ledger: an automatic factual record of every watched morning, night by room, consultable any time. The ledger records what happened and the keeper's strikes and circles record what it means, so marks-are-never-verified survives intact. Without it the drain flag says a conclusion is wrong while the mornings that produced it are gone, and her taught method cites evidence the keeper cannot look up; Obra Dinn and Golden Idol both keep all evidence revisitable forever, and thirty to fifty four-signal mornings across six spirits is beyond working memory by design. The save already holds every morning seen, so the ledger is existing data rendered in the book's voice. From ticket #24 and its game-design gate, which caught the gap as its one DECIDE-NOW.
- **2026-08-09** Each spirit's page shows its current drain, the chosen hint channel, coarse and diegetic: it eases as the mornings settle axes, confirming progress and flagging a wrong conclusion, and it never names a value or touches the keeper's marks. Refusing the channel was rejected because drain relief already moves player-facing warmth, so refusal only makes the information illegible. Easing is shown as a witnessed edit, the old figure struck and rewritten in the morning, never a number the keeper must difference across nights, because the displayed drain also carries the season and an unattributable delta cannot flag anything; how relief and season compose legibly is #15's. From ticket #24 and its gate reviews.
- **2026-08-09** A completed entry is one fixed four-slot sentence in the book's register, hers and the keeper's alike, the phrase fragments riding each trait's presentation. Golden Idol proved slots in a sentence read as language rather than stats, and a fixed formula keeps a right answer from ever being phrased wrongly. Rejected: composed epithet names, which go mad-libs across 93 pool values, and a styled stat list, which is the thing the book must not read as. The haunt fragment gives loose spirits the keeper's own verb, keeping the parlour, which carries spirits-are-failed-keepers as pure texture; the 2026-08-06 rejection of keeping was about interface buttons and does not bind presentation fragments. From ticket #24.
- **2026-08-09** The book teaches by worked example in two layers and contains no instructions anywhere: her completed entries show the formula and the destination, and her unfinished last entry shows the method mid-flight, struck values and a circle each with a one-line margin note naming the evidence that made it, the name line half filled and cut off where she died. The worked example withholds and the margins disclose: no generator rule is stated outright in the example, while her margin notes plainly give the room rule and trace purity, because the difficulty dial is the economy by measured decision, #15's solver-tuned prices only transfer to a human who knows the structure, and trace purity withheld sends keepers hunting a fifth axis that does not exist. Obra Dinn disclosed its whole structure and kept the difficulty in the evidence. A page of her method stays unwritten, free for opening 2's fiction. From ticket #24.
- **2026-08-09** Each year's section of the book opens on a roll, one ruled line per spirit stirring that year: a line takes the trace at sighting and the name at naming, so roster size, the naming count, and the win condition are one glance at one page, never a meter. Her line is the sighting rule extended to the roll, ruled at her coda sighting and not before, so the climactic naming lands in the book's own device without a spoiler. Opening thresholds are disclosed as her margin notes beside the roll, fixed templates over the same numbers #14 tunes, so the lore is always true and tuning never edits fiction; that her notes attest all three thresholds means she reached the third opening before she froze, a staging fact for #20. How the keeper knows six on night one is #20's to stage. From ticket #24 and its gate reviews.
- **2026-08-09** A spirit's page is ruled at sighting from the pools in force at its arrival stage, and values from later openings are simply absent, never grayed. Absence is the honest fiction, the page could not anticipate the wing, and it cannot be mistaken for the keeper's own strike. Graying is the named fallback if playtests show keepers probing new rooms on old spirits anyway. Extends the 2026-08-08 arrival-stage narrowing decision to the interface. From ticket #24.
- **2026-08-09** On a return the book keeps last year whole: the old roll and pages stay exactly as the keeper left them, readable and plainly closed, and the return rules a fresh roll for the new year. The keeper's abandoned years accrete beside her closed sections in the same register, which carries spirits-are-failed-keepers as texture in exactly the role #5 gave the margins. This is the fresh cross-run-state decision #23 said the profile would not grow without: kept sections are mechanically inert but they are state beyond the one head-start flag, and their storage shape goes in front of the tech-architecture gate when the book is built. From ticket #24 and its product-vision gate, resolving #23's DEFER.
- **2026-08-09** The book's teaching is the design's riskiest assumption and gets tested before it gets built: her completed and unfinished entries mocked as static pages in front of three to five fresh testers ahead of a scripted first morning, scheduled through #20 with a date, because drift to later is the documented default. The same session probes whether absent pool values read as closed at arrival or as the book forgetting. From ticket #24's gate reviews. Struck 2026-08-10: Mark cancelled the teaching test outright, no tester session happens, and nothing downstream expects one. The graying-versus-absence question keeps its named fallback in the book design and needs no session.
- **2026-08-11** The night is set in the room itself, never in a form: glowing placement spots pop their options where the keeper clicks, and choosing which room to stand in is the first decision of the night. This settles the open centerpiece question #20 was created to answer, whether the interactive surface is forms around one picture or standing in the room. From ticket #20's storyboard.
- **2026-08-11** Set-the-night and its summary are one surface. What gets placed pops into a list beside the warmth meter and the cost climbs live; candle and bowl are the two a night requires, the ward is optional, "Set the night" lights the moment the required two are down, and the keeper can still walk to the door and add salt afterwards. A separate confirm screen would have split one decision across two surfaces and hidden the price at the moment of choosing. From ticket #20's storyboard.
- **2026-08-11** The book is one held key away during set-the-night, its hint always on screen as a chip. Consulting the evidence is part of setting the night, not a mode change out of the room. From ticket #20's storyboard.
- **2026-08-11** The night is a short hold and then candle-out total blackout: everything the night says lands in sound, a knock at the threshold, a step on the stair, and then dawn. This is the beat sound design exists for, and it keeps the 2026-08-08 night contract intact because an unwatched room still shows nothing. From ticket #20's storyboard.
- **2026-08-11** The morning is evidence with no narration, and each signal names itself on hover, the agreed backstop for a read that must land in seconds. From ticket #20's storyboard.
- **2026-08-11** A commit frame sits between marking and naming: one value circled on every axis is the gate, and only then does the submit verb light. Whole-or-not-at-all lives in that gate rather than in a rule the keeper has to remember. The verb itself is a placeholder. From ticket #20's storyboard.
- **2026-08-11** A completed entry writes itself out in the keeper's hand as one four-slot sentence, and if the name is true the spirit's byname takes the page header as it lands. The 2026-08-09 four-slot decision said what the sentence is; this says what naming looks like on screen. From ticket #20's storyboard.
- **2026-08-11** Named spirits are visible as translucent shades working the rooms they keep, hover for the name. This is the first sight of who the spirits actually are, and it is the kept-rooms carrier of spirits-are-failed-keepers doing its work without text. From ticket #20's storyboard.
- **2026-08-11** The roll shows six bare lines from the start, "six are stirring" with no counter. Obra Dinn's manifest and Golden Idol's visible blanks both show scope while hiding contents; it mirrors her filled page so the book teaches by worked example; and it makes the winter priceable from night one. The house is what gets discovered, never the census. How the keeper knows six on night one, which #24 handed to #20 to stage, is answered here: her roll page shows it. From ticket #20's storyboard.
- **2026-08-11** The Leave offer speaks in survival terms, not in points, and it must name both sides of the choice: what going buys, what staying could still get, and that past a certain cold there is no going at all. An offer that prices only the cost of leaving gives the keeper no reason to stay. The withdrawn offer is a buttonless dead notice under frost, never a dialog. Wording is placeholder; #15 owns thresholds, the window, and the final lines. From ticket #20.
- **2026-08-11** "Wintering out" is retired from all player-facing words. The beat is "Taking the road." From ticket #20's storyboard.
- **2026-08-11** The frozen keeper is visibly a different person from her, the knit cap against her bun in the placeholder art. Real art may make them non-binary, but different regardless, because the freeze image only carries the line of failed keepers if the body is legibly not hers. From ticket #20's storyboard.
- **2026-08-11** Freezing plays as an animation rather than cutting to a still: the keeper shakes where they stand, goes down to their knees, and settles into the pose the intro image found her in. Mark's read, taken. A cut to a still reads as a game-over card; the collapse read as the house taking you the way it took her. From ticket #20.
- **2026-08-11** The withdrawn offer is staged in an order that cannot be misread: Stay is pressed and the card goes, nights are played and the meter pays for each, and only then does the keeper walk out to the road and find the notice dead. The design turns on the notice being found one night too late rather than appearing the instant you decline to leave, and the first staging failed to carry that. When and how road-checking enters the day stays #15's, still open. From ticket #20.
- **2026-08-11** The win plays whole and quiet before the offer interrupts it. From ticket #20's storyboard.
- **2026-08-11** "Stay for tea?" is answered through the same interaction grammar as every other night, no special interface. It is the one deliberately loud moment in a quiet game, and it lands because the grammar around it never changed. From ticket #20's storyboard.
- **2026-08-11** The coda runs the exact same experiment loop as the whole game, in the last cold room: no candles-only economy, no new rules. It should be hard to fail, and how it is made hard to fail is design still owed. From ticket #20's storyboard.
- **2026-08-11** Consistency fixes across the record: the lure is honey rather than "shine", the ward is iron rather than "running water", and the aversion named in the book frames is iron. From ticket #20's storyboard.
- **2026-08-11** The scripted playthrough plays two years with one marked aside: year one runs arrival through the Leave offer and ends taking the road, the aside plays the withdrawn notice and the freeze, and the return opens year two, which runs to the win, tea, coda, and her. Both endings and both years are legible in one watch without pretending a single run contains them all. From ticket #20.
