...
| `trace-water-used` | Water drawn and gone | Russia | attested | The bathhouse spirit leaves the room used: the water gone, the birch branches scattered, the soap moved. Pairs with `lure-fresh-water` the way `trace-emptied-bowl` pairs with the food lures, and the sign is that the thing left out was consumed. **Renamed and narrowed on checking.** It was `trace-wet-marks`, which named a sign no source records. Henderson does not say English water hags leave water behind them; his Peg Powler and Jenny Greenteeth are drowners, and the only residue in the entry is froth on the river. The Russian bathhouse is the whole of the attestation, and what it attests is water used up rather than water left. | Ivanits 1989 † |
...
- **Wet marks left behind, as a trace of any kind.** Cut on checking, and the whole idea went, not just its English half. Henderson's Peg Powler and Jenny Greenteeth pull people into the water and drown them, and the only residue named is froth on the river itself. The Russian bathhouse material, which was the other half of the row, records the opposite sign: water *used up* rather than water left. So there is no source for water found where it should not be, and the row became `trace-water-used`, which is what the sources actually describe.
