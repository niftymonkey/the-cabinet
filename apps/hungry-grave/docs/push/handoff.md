...
- **Cloud facts (official docs, 2026-09-07):** cloud sessions share the account's usage limits; fresh Ubuntu VM with node 22, pnpm, gh, jq, docker, Postgres pre-installed; repo `.claude/` loads, personal `~/.claude` does not; no local `.env`; the environment's variables are copied once at session start (edit them, then start a new session); setup script runs once and the VM is snapshotted; `gh` is signed by a proxy without a token; GraphQL is limited to PR operations, use `gh api repos/...` REST. `claude --cloud "<task>"` from the worktree launches a session on the pushed branch; `claude --cloud -p "<message>" <session id or url>` queues a follow-up into a running one; `claude --teleport` pulls a cloud session back. The Agent tool's `isolation: "remote"` does NOT go to the cloud on this account.
