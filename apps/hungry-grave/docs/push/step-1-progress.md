...
## 14. Gate fixes after slice 10

Commit `6b6f01671d`, two findings from the tech architecture gate applied. `pnpm typecheck` and `pnpm vitest run` inside the app, then `pnpm verify` at the worktree root: 109 test files, **1413 passed, 10 expected fail, 3 todo (1426)**, against slice 10's 1405 passed. The eight new tests are all in the fence file. `GOLDEN`, `WITNESS_VERSION` and `FORMAT_VERSION` did not move and are not in the diff.

**`TERRITORY_CAP` moved from `caps.ts` to `lines/territory.ts`, with its ruling comment.** The gate read the `_CAP` exception as two thirds honest and it was right. `caps.ts` called `SKULL_CAP` and `WISP_CAP` safety nets in its own words and called `TERRITORY_CAP` "a gameplay rule and not a safety net" in the same breath, and that row decides how long a trail of claimed ground is, which is Territory's tuning and nobody else's. It sits with `TERRITORY_PERIOD` and the radius ladder now, which is what its own derivation is measured against. Readers repointed: `invariants.ts` (the patch pool check), `StormRenderer.ts` (which already imported from `lines/territory`), and the three test files that pinned the number. `caps.ts`'s header says what it now holds, that every capacity declared there is a safety net and a capacity deciding how a line plays lives in that line's module.

**The fence's exception is two names now and not a suffix.** `SAFETY_NET_CAPS` is `['SKULL_CAP', 'WISP_CAP']` and nothing else is excused inside `game/caps.ts`. A `BELL_REACH_CAP` written into `caps.ts` fails, which the old form would have waved through, and there is an assertion for exactly that. A fourth control asserts the two excused names are really declared in `caps.ts`, so a safety net renamed or moved out leaves the exception list excusing a name that is not there rather than reading green.

**`isFirstOffer`'s sentence is a fence.** `offer.ts:84-97` reads the run's first offer off the drops cursor, sound only while the offer is the one thing that moves it, and that rested on a comment. The walk covers `src/game`, which is the "sim" the sentence names, and fails any module but `game/offer.ts` that reaches `streams.drops`. Four spellings are caught: `streams.drops.next`, the bracket form `streams['drops']`, a bare `streams.drops` taken into a local, and a `const { drops } = state.streams` destructure. One member is excused everywhere, `.drawn`, because it is a getter over a counter and reading it cannot advance the cursor. A control asserts `offer.ts` still reaches `streams.drops.nextInt`, so a drawer that stopped drawing cannot leave the fence guarding an empty claim.

**The second-reader walk found no second drawer, and two non-drawing readers that are named rather than fixed.** `invariants.ts:188` checks `state.streams.drops.drawn` is finite and `src/dev/digest.ts:151` folds the same cursor into a reading. Neither is a draw and neither can be: `.drawn` is a getter over a counter in `rng.ts:114-116`. That is why the fence is over draws and not over mentions, and the `.drawn` excusal is written into the helper's comment with both citations. Nothing was edited on their account. `witness.ts:262` folds every stream's cursor through `run.streams[name].drawn`, a computed index no text match can tell from a draw; that and an alias of the whole streams record are stated as outside the fence rather than claimed closed.

**Both fences were proven against the real tree and not only against their controls.** A `BELL_REACH_CAP` planted into `caps.ts` and a drops draw planted into `storm.ts` each turned the corresponding walk red with the module named in the failure; the bracket and destructure spellings were planted separately and both came back named. All plants were reverted before the commit and `git status` was checked clean of them.

**CodeRabbit, `-t uncommitted`, one major finding, applied, none declined.** The first run said the walk saw only dot notation, so `state.streams['drops'].nextInt()` and a destructured alias would both have walked through. Both are matched now, with a regression test each, plus a third asserting that `run.ts`'s `streams: { ..., drops: stream(seed, 'drops'), ... }` object literal is read as a literal and not as a binding, because a binding matcher that could not tell those apart would fail the module that creates the stream. The re-run reported zero findings.

**One formatting note for the next slice.** Adding `import { TERRITORY_CAP } from './lines/territory';` to `invariants.ts` collapsed the neighbouring `./caps` import back onto one line under Prettier, so `pnpm format:check` failed on a file the edit had only added a line to. `npx prettier --write` on that file is the whole fix; it is worth running before `verify` after any import-block edit.

## 15. The tip review

One CodeRabbit review of the whole branch against main, sixteen findings, six fixed and ten declined. Two commits: `a02f5980dc` for the one code finding and `e22a2111df` for the five record corrections. `pnpm typecheck` and `pnpm vitest run` inside the app, then `pnpm verify` at the worktree root: 109 test files, **1413 passed, 10 expected fail, 3 todo (1426)**, the same count section 14 recorded, which is what a slice that replaces one assertion and edits five records should produce. `coderabbit review --agent -t uncommitted` on the code change reported zero findings. `GOLDEN`, `WITNESS_VERSION` and `FORMAT_VERSION` did not move and no production module is in either diff.

**Fixed.**

- **`bell.test.ts:255`, the one-live-toll check was a tautology** (`a02f5980dc`). `state.lines.ring === null || typeof state.lines.ring === 'object'` is true of every value the field can hold, so the test's own promise, at most one live toll, was asserted by nothing. It now reads the toll: a live ring's `ticks` is always below `BELL_EXPAND_TICKS`, and a ring that is not the object the last tick held is asserted to have been born onto an empty field, which is what "at most one" means when `tollClock` overwrites `lines.ring` unconditionally. A count of tolls born guards against the loop passing over an empty set. Both halves were proven against the real tree rather than only against their own logic: `expandToll`'s clear was planted at `BELL_EXPAND_TICKS * 3`, which turned the ticks bound red at tick 224 and the empty-field check red with the live toll named, and the plant was reverted before the commit.
- **`CONTEXT.md`, the Overflow entry named a branch ADR 0034 retired** (`e22a2111df`). It said "a maxed weapon line's drop or growth past the ceiling converts to score, radius, or reservoir charge". ADR 0034 rules that branch dormant in as many words ("overflow itself stands for growth past the ceiling"), and `swallow.ts:101-103` pays overflow to score alone, with growth and reservoir paid separately from the same amount. The entry now says growth past the size ceiling converts to score, names the maxed-line branch as dormant, and says what a no-option body actually pays.
- **`step-2-stage-floor-dispatch.md:591` contradicted its own cycle argument** (`e22a2111df`). "The set piece's live in `setPiece.ts`" against lines 210, 297, 545, 852, 1053 and 1065, every one of which puts the pour's rows in `rows.ts` because `peakArrivals` reads them and a `rows.ts` reaching `setPiece.ts` closes the cycle `boundary.test.ts:17-19` guards. It is a leftover from before the tech gate's "pour rows live in the data module" finding. Line 591 now carries the same rule as the rest of the plan.
- **`stage-floor.md:65`, the Vigil's headroom was off by one** (`e22a2111df`). A ceiling of four against an authored steady state of about two is two bodies of headroom, not one. The ceiling itself is a data row the dispatch plan states twice (`step-2-stage-floor-dispatch.md:258,967`), so the prose is what was wrong and the prose is what moved.
- **`step-1-progression-dispatch.md:15`, the first-offer contract said "always"** (`e22a2111df`). Section 8's gate correction rules the fixed shape a preference and not a demand, and the tree implements it that way (`offer.ts:109-122`) with a test each for the fresh run (`offer.test.ts:192`) and the part-built one (`offer.test.ts:208`). The definition sentence and the planned test 8 both name the fresh run now and both point at section 8's exception.
- **`handoff.md:21` said the step 2 records were uncommitted** (`e22a2111df`). They landed as `97af4ae66d` before slice 7, which the same paragraph says two sentences earlier. The status clause, the "two untracked files" wording and the instruction to commit them are corrected; the conditional check on the ceilings and the LAME header stands, because it is a check and not an instruction to commit.

**Declined.**

- **`offer.ts:256`, decrement `bankedOffers` only when a body was placed.** Real as a latent defect and declined here because step 2 owns the whole family. `openBanked` spends the bank before `standOffer` runs, so an offer whose every body the corpse pool refused would spend a banked offer and put nothing on the field. It is unreachable in this tree: `claimSlot` (`corpses.ts:160-177`) evicts rather than refuses, and a refusal needs all 200 slots holding live non-decaying treasure at the one moment a take or a loss has just freed three of them. The step 2 plan names both halves of this at `step-2-stage-floor-dispatch.md:871` and lands them together at slice 5, with the corpse cap that first makes a refusal ordinary: an all-refused offer banks rather than disappears, and a recoverable fault identity with its own wire code records the refusal. The guard on its own is a correct partial, but a silent one, and a bank preserved with nothing said breaks the standing rule that nothing abnormal is ever silent; worse, step 2 adds a per-tick opening site (`:869`), so a silently preserved bank would retry every tick and report nothing. No red test can be written for it today without fabricating a pool the sim cannot reach. It goes whole to step 2 slice 5.
- **`run.ts:157`, derive `isBirthrightLevels` from the run's roster.** No difference against this tree, and the change would lose information. `BIRTHRIGHT` is the skull stream alone (`roster.ts:14`) and `implementsLines` (`roster.ts:42`) refuses a roster missing any birthright line, so `birthrightLevels(roster)` equals `birthrightLevels()` for every roster a run can field, and `resolveStartingLevels` (`startingLevels.ts:87-88`) zeroes every line outside the recorded roster, which is what the full-pool comparison reads. Iterating the roster instead would make a pinned level on an out-of-roster line invisible, so a tape would record that run as unpinned.
- **`scripts/store-smoke.mjs:29`, name the blob yourself so cleanup can find it.** Step 6 (#100) owns the store, and this is a dry-run script that cannot be exercised here: verifying any change to it needs `BLOB_READ_WRITE_TOKEN` against the real store. The failure it names is one orphaned test blob when a `put` loses its response, in a dev store, on a script no build imports.
- **`step-2-stage-floor-dispatch.md:301`, `SET_PIECE_HP` against tests 18 and 117.** Not a contradiction and not mine to settle. Test 117 is that a full build cannot kill the source during its pour; test 18 is what happens if it dies at all, which the mechanism still has to answer. The plan's section 9 item 3 (`:1013`) already records this as a stated assumption with the reversal named ("the health row drops below the pour's damage"), and `stage-floor.md:134` hands it to Mark to overrule. Step 2 (#97) owns the row.
- **`step-1-progression-dispatch.md:551`, the no-option reward contract.** The plan reconciles it at that exact line rather than contradicting anything: it names the narrower reading, says why the alternative ("adding score silently with no body on the field") was rejected, and notes the two readings coincide at the ceiling. ADR 0034's "converts to overflow" is the shorthand and `game-concept.md:38`'s "overflow, bonus radius, score and belch charge" is the whole swallow, which is what `swallow.ts:94-104` pays.
- **`step-1-progression-dispatch.md:147`, round `carriersScheduled()` up.** Wrong against the tree. `carriers.ts:53-55` is `Math.ceil(carriersForFullBuild() * CARRIER_SLACK)` and returns 25 for 19 times 1.3, which is the number the plan already states.
- **`stage-floor.md:118`, the two set-piece policies against ADR 0053.** Wrong about what they are. They are bot policies in `src/dev/bot.ts`, where `unloadedPolicy` and `belchingPolicy` already carry the Wall's property the same way, and the step 2 plan lands them at slice 10. ADR 0053 governs the step 3 harness, a different instrument, and neither the record nor the plan calls these two error configurations.
- **`stage-floor.md:292`, give the Banshee's phase the second music alias.** It would move the music change to the start of her fight, and decision 22's amendment rules the two changes at the Banshee's **death** and the eye opening. The Banshee's phase naming the Procession's alias is what puts the change at her death, and `audio.ts:21` makes `play` a no-op on an unchanged alias.
- **`open-tickets.md:311`, fix the `#38` link fragment.** The string is a verbatim quotation of ticket #26's body, marked as such in the line, so editing the link inside it would falsify the quote. The suggested fragment does not match this document's own heading either (`## #38 Dress the tracer for Halloween: the art and theming pass`).
- **`section-feel-consent-and-rung-restore.md:93`, reformat the quoted blocks.** Each quoted paragraph is already on its own unwrapped line, which is this repo's markdown rule; the lines are short because the shipped UI strings they quote are short. The rest is rendering taste on a research record.
