...
**One record still puts the HUD inside the field frame and it was left alone.** ADR 0061, Mark's ruling of 2026-09-09 that six words move to the genre's vocabulary, identifies its fifth term as "the readout inside the field frame", which is a clause describing which readout was renamed rather than a placement ruling. **Amending a second Mark-ruled ADR is not this commit's ruling to take**, R9 names `CONTEXT.md`'s entry alone, and it is recorded here for his read. `six-words-realigned-to-the-genre.md` quotes both that ADR and the old HUD entry, and a research record quoting the state at its own date is left as it stands.
...
5. **Human (Mark), open and blocking nothing.** ADR 0054 is his and the amendment is ours: he reads it on the branch and it is his to overrule before merge.
