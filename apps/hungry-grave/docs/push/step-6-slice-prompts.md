...
2. **The progress note is `apps/hungry-grave/docs/push/step-6-progress.md`, a new file, not step 5's and not round two's.** Slice 1 creates it with sections 1 to 5 in the same shape as `step-5-progress.md`'s sections 1 to 5, read for the shape and never copied for the content: 1 slices committed, 2 the version ledger, 3 `GOLDEN` moves, 4 the fences, 5 the cross-slice facts. **Sections 6 onward are one per slice in run order: slice 1 is 6, slice 2 is 7, slice 3 is 8, slice 4 is 9, slice 5 is 10, slice 6 is 11, slice 7 is 12, slice 8 is 13.** Step 5's note and step 4's are read and never appended to.
...
Slice 1 of the tuning-record step. Every slice after you writes a seam whose name is a glossary word, and two of those words do not exist yet. **The code rules say a public seam carries the domain glossary's word and a defined term is fully descriptive on its own**, so the words land before the seams do rather than being back-filled once seven commits already spell them. `Rig` and `Configuration` both entered the glossary the same way when the harness gained them (`CONTEXT.md`, ADR 0053), which is the precedent this slice follows.
...
**Third: you write no ADR and amend none.** The new ADR for the tuning record and the starting-condition record on the run is the orchestrator's and is filed before slice 2 dispatches. **Your entries cite it by number if it exists at your tip and by nothing if it does not**; an entry that cites a record you cannot open is a stop and report.
...
After this slice the glossary holds two entries it did not: a reader who has never seen this step can read `CONTEXT.md` and say what a tuning record is, what a candidate is, and how the two differ from a rig, a configuration and a batch. **The `Batch` entry no longer forbids the word the step's own command is named with**, and says what a sweep is instead. **A new progress note exists** with five empty cross-slice sections and one filled section of its own, so slice 2 appends rather than inventing a file.
...
**(b) The two entries**, in `CONTEXT.md`'s "The build" section, each in the section's own voice: a bold term, a definition that is fully descriptive on its own, what it is not, and an `_Avoid_` list. **Place them beside `Rig`, `Configuration` and `Batch` rather than at the section's end**, because a reader meets the harness words together. *Tuning record* says it is the numbers a batch reading can move, grouped by the module that owns them, resolved once at the start of a run and carried on it, and that it is not the caps and not the safety nets, which are a derivation and a bug detector. *Candidate* says it is a named tuning record a batch or a play runs under, that the name is what a batch folder and a report carry, and that a candidate is a starting condition on exactly the terms a rig is and never a description of the hand that steered.

**(c) The `Batch` amendment**, in place, in the dated triple form.

**(d) The progress note**, `apps/hungry-grave/docs/push/step-6-progress.md`, created with the five cross-slice sections and section 6.

**(e) CodeRabbit CLI, one iteration**, `git add` every changed and new file by path first, then the docs commit. Something in the shape of `docs(hungry-grave): the glossary gains the tuning record and the candidate, and a sweep is not a batch (#142)`.
...
- **Every other glossary entry**, its definition and its avoid-list, `Rig`'s amendment included. **`Signal lock`'s avoid-list keeps "override"**, which is why this step does not use the word.
...
**Three things are already known to be a stop:** any file under `src/` or `scripts/` in your diff; an entry citing an ADR that does not exist at your tip; and `pnpm verify` red on the committed tree. **And three things are ruled rather than open:** two entries and no more, the words "dial" and "override" banned, and the `Batch` avoid-list amended rather than the script renamed. **A gate finding or a measurement arguing against any of the three is filed in the note, never applied.**
...
- **Any entry other than the two named and the one amendment.**
...
**Second: the seed stays first and stays positional.** `createRun(seed, conditions?)`. The seed is the run's identity in a way none of the rest is, `rollSeed()` is its default, and `createRun(SEED)` is the whole call at three sites (`src/dev/digest.ts`, `scripts/frame-budget.ts`, `src/app/screens/FrameBudgetScreen.ts`). **Keeping it first is what keeps `digest.ts` out of this commit**, which is the surest proof the golden's own run did not move.

**Third: the record is the starting conditions and its name is the domain's.** Five fields, exactly the five the signature carries today: the starting size, the starting levels, the roster, the signal lock and the starting score. **`StartingConditions` is the recommendation and the concept is the ruling**; if a truer name reads better with all the callers in front of you, take it and say why in the note. It is **optional as a whole and every field inside it is optional**, each resolving to exactly the default that parameter resolves to today, because the resolution of an absence to a value is `createRun`'s job and never a caller's (ADR 0027). The record's own module is `src/game/` beside `run.ts`, or inside `run.ts` if it has no second export; **a file is a concept module and the concept here is a starting condition**, so if it earns a file, it gets one.
...
**Sixth: `TapeHeader` is not touched at all.** The header's own move is slice 6, it costs a `FORMAT_VERSION` bump, and the bump is taken once in the slice that last changes the layout. **Nothing under `src/tape/` changes in this slice except `playback.ts`'s `runFromHeader`, which builds a `StartingConditions` from the header's existing fields and passes it.**
...
4. `apps/hungry-grave/CONTEXT.md`, the entries **Rig**, **Configuration** and the two slice 1 added. You amend none of them unless one has gone false, and you say in the note what you decided and why.
5. `apps/hungry-grave/docs/adr/0027-*`, **for why a header records a resolved value and never an absence**, and `docs/adr/0053-*` for what a rig is and is not. The new ADR the orchestrator filed for this step's shape, if it is at your tip: read it and say in the note that you did.
6. The tree, by name and never by line: `src/game/run.ts` **whole**, and `createRun`, its JSDoc and `RunState` above all, **because that JSDoc already explains why each of the five is in the signature and it is the material your record's own JSDoc is built from**; every call site, which is `src/dev/harnessRun.ts`'s `playHarnessRun`, `src/dev/floorLadderWalk.ts`'s `stagedRun`, `src/dev/digest.ts`'s `runScenario`, `src/app/screens/game/runSession.ts`'s `begin`, `src/app/screens/FrameBudgetScreen.ts`, `src/tape/playback.ts`'s `runFromHeader`, `scripts/record-conditioned.ts`'s `recordTape` and `scripts/frame-budget.ts`; `src/dev/rigs.ts` **whole**, 123 lines; `src/app/seedFromUrl.ts` **whole**, for the five parsers whose values `begin` assembles.
...
**(c) The record and the signature.** `StartingConditions` with its five optional fields, each carrying the JSDoc paragraph `createRun`'s own JSDoc already gives that parameter, moved rather than rewritten. `createRun(seed, conditions?)`, its body resolving every absence exactly as it resolves one today.

**(d) Every call site, in one commit.** Eight outside the tests, in seven files, listed in the read list. **Say in the note that you checked all eight and name any you found that the list does not.**
...
- **`TapeHeader` and everything under `src/tape/` except `runFromHeader`'s body.** The header's move is slice 6's and it costs the step's one `FORMAT_VERSION` bump.
...
`src/game/run.ts`: `createRun` taking a seed alone and rolling a fresh run; taking a seed and a record and starting from it; taking a record naming some fields and resolving the rest to the same defaults it resolves today. `src/dev/rigs.ts`: a rig as a name plus one record, applied whole, and `rigOf` still banding a tape by the two facts a header carries. `src/tape/playback.ts`: `runFromHeader` building the record from a header's own fields. `scripts/record-conditioned.ts`: the command line producing the record, with every refusal and the warning unchanged.

### Module boundaries

**At most one file is created and none is deleted, merged or split.** If `StartingConditions` earns its own file it goes in `src/game/` beside `run.ts`, named for the one concept it owns, with the type and nothing else; if it does not, it lives in `run.ts`, whose concept it plainly serves. **No import direction changes**: `src/game` stays dependency-free and reaches only itself, `src/dev` reaches `dev`, `game` and `tape`, `src/tape` reaches `tape` and `game`. **No new library enters.** The record is a data type, so it is named for the domain noun and carries nothing about what kind of type it is; **`Rig` holds one rather than extending one**, because the rules forbid an intersection where an interface belongs.

### The planned test list

1. *A run started with a seed alone rolls the same run it rolls today*, every resolved default read off the run.
2. *A run started with a record holding one field resolves every other field to its own default*, one test per field, which is five.
3. *A run started with a full record starts from every one of its five facts*, read off the run rather than off the record.
...
**Third: the rows keep the form their constants have, which is what makes the default identical by arithmetic.** Four of the five score constants are stated as multiples of `TRASH_KILL_SCORE` on purpose, and their JSDoc says why: "the sentence a flat cap exists to make sayable is 'a hit at the floor costs you twenty kills' and a bare two thousand says nothing about the income it comes out of". **So the record holds the multiplier, not the product**: the kill's own unit, the cap in kills, the boss health one kill is worth, the source kill in kills, the meal in kills. **Each reader does the same arithmetic it does today, off the record's row instead of off the constant**, and the resolved default therefore equals today's value by arithmetic rather than by a second copy of a number. **A row that restates a product is a stop**, because two spellings of one figure is the defect the whole record exists to remove.

**Fourth: the record's module imports nothing and nobody imports it from inside the core.** `src/game/tuningRecord.ts` is the recommendation and the concept is the ruling. It is the `signalLock.ts` shape: a type and a resolver the sim, the header and playback can each own without any of them importing a consumer. **The cycle guard shapes it:** `caps.ts` imports `waves.ts` today, so if `waves.ts` imported the record module the group would close, which is exactly why every reader takes the record as an argument and never imports it. **`KNOWN_CORE_CYCLES` stays empty and you prove it by running the guard.**

**Fifth: the resolver takes a partial and answers a complete record, and it refuses nothing.** `resolveTuning(partial)` fills every absent row from the default, group by group. **Parsing a raw name a person typed is the edge's job and not this module's** (slice 7), so this function's input is already typed: it is `Partial` over the record's own shape, and there is no such thing as an unknown row reaching it. **Repair by origin is why**: values our own code produced are never repaired, because a bad one is a bug.
...
4. `apps/hungry-grave/CONTEXT.md`, the **Tuning record** and **Candidate** entries slice 1 wrote, and **Rig**, **Configuration**, **Reading** and **Batch** beside them. Your type's name and your rows' names are checked against those entries.
5. The new ADR the orchestrator filed for this step's shape. **Read it and name it in the note.** If it is not at your tip, that is a stop and report.
...
**(g) The fences.** Run every one by title and name them in the note, plus the core cycle guard with `KNOWN_CORE_CYCLES` still empty. **The new module must import nothing at all**, which the `game` row's `mayImport: []` already asserts and which you confirm by running the fence rather than by trusting this sentence.
...
7. **The fences**, green, each by title, plus the core's cycle guard with `KNOWN_CORE_CYCLES` still empty.
...
**Second: the record rides on the starting conditions.** `StartingConditions` gains one optional `tuning` field, resolving to the default inside `createRun` the way every other field resolves (ADR 0027's own rule about a resolved value, applied to a starting condition). **`RunState` carries the resolved record as a readonly field and nothing writes it after the first tick.**

**Third: the caps become functions of the record and the run carries their answers.** `caps.ts`'s `MOB_CAP`, `MOB_FIRE_CAP` and `CORPSE_CAP` stop being constants and become named derivations taking the record: the mob cap and the corpse cap read `stage.quietIntervalMinimumSeconds` through `directedInside`, and the mob-fire cap reads nothing in the record today and takes it anyway only if it genuinely needs it. **A derivation that needs no row takes no argument**, which is the cited-future rule applied to a parameter. `RunState` carries the three answers in one readonly group beside the pools, and **every reader that read a module constant reads the run's field.**

**Fourth: `stormTargets`' scratch array moves onto the run, and this is the one part that is not mechanical.** `SLOTS` is a module-level array sized `MOB_CAP + 2`, allocated at import, and `scripts/frameBudgetCaps.ts`'s own JSDoc names it as the reason the bench raises its ceiling before the game is imported at all. **A module-level array cannot be sized per run**, so the scratch is built in `createRun` at the run's own mob cap plus two and carried on `RunState` beside the pools, which is exactly what the pools already are and what the array's own JSDoc already asks for: pre-allocated and refilled in place so that a tick allocates nothing. **`frameBudgetCaps.ts`'s sentence goes false the moment you do it and is rewritten in the same commit**, which is what step 5's slice 8 did to `rigs.ts`'s own comment.

**Fifth: the frame-budget bench aliases the derivations, not the constants.** `scripts/frameBudgetCaps.ts` exists so a synthetic field can stand at a size the shipped caps refuse, and it does it today with two `let` bindings and a named `sizePoolsFor`. **It keeps its concept and its named setup function exactly**; what changes is that it aliases the cap derivations rather than two numbers, so a bench run's caps are the bench's and a game run's are the record's. **`FrameBudgetScreen.ts`'s two comparisons and its message read the run's caps** rather than the module's.
...
4. **ADR 0056 as the orchestrator amended it for this step**, which is the record that makes the caps derivations of the content, and **the new ADR for the record on the run**. Read both and name both in the note. **If either is missing at your tip, that is a stop and report.**
...
**(f) The scratch array onto the run.** Built in `createRun` at the run's cap plus two, carried beside the pools, with `slotAt`'s own JSDoc rewritten to say where the size comes from now. **`frameBudgetCaps.ts`'s JSDoc sentence about raising the ceiling before the game is imported is rewritten in the same commit.**
...
**(k) The progress note**, section **9**. Beyond the contract's list, say: the field count and what the list below got wrong; the three caps before and after; the scratch array's new home and the two comments that went false; what the bench looks like now and what you ran; the new fence by title; and the four constants and `GOLDEN` all named as read off your own tip.
...
4. *A run's storm scratch is built at the run's own mob cap plus two*, counted off the run.
...
**What this slice is expected to turn red.** Every test that imports a cap constant, which is `caps.test.ts` and whatever else the grep finds. **A realistic count is 12 to 20 files**, and the honest expectation is the high end: the readers outside `caps.ts` are `corpses.ts`, `mobs.ts`, `mobFire.ts`, `stormTargets.ts`, `invariants.ts`, `run.ts`, `FieldRenderer.ts`, `FrameBudgetScreen.ts` and `frameBudgetCaps.ts`, plus their tests. **Say in the note whether that list was right**, because step 4's slice A is the warning that it may not be.
...
**Third: `MOB_TYPES` is the delicate one and its rows stay where they are.** The four `scorePayout` figures are stated as multiples of `TRASH_KILL_SCORE` and the table is a mob type's own stats, which `tuning.ts`'s own opening comment says is deliberately not this file's business. **`MOB_TYPES` is eligible for the record and is not carried in this step** (this file's header says so), so the table keeps its rows and its multipliers; what changes is that the multiplier is applied against the run's kill unit rather than the module's. **The seam is where a kill pays**, not where the table is declared: `payKill` reads the run's record and the row's multiplier. **If that turns out to require restating the table's four numbers as bare multipliers, say so and stop**, because that is a table change and this slice is not permitted one.

**Fourth: the purse comes off the record at the grant and the `Section` row loses its `purse` field.** `directorGranted` is the one place a purse is read (`stage.ts`), the section table is authored content rather than a magnitude, and a purse in both places is two spellings of one fact. **The record's stage group is keyed by section name and the grant asks it**, and a section with no purse still grants nothing rather than carrying the last section's over, which is ADR 0056's own rule and is the behaviour that must not move. **`SECTIONS` has six readers outside `stage.ts`** and none of them reads `purse`; **check that yourself rather than trusting this sentence**, and if one does, that is a stop and report.

**Fifth: the constants stay exported where something outside the core still reads them.** `src/dev/rigs.ts` reads `SCORE_BLEED_CAP` for the ladder row's starting score, which is a harness starting condition resolved off the default and not off a run. **That is correct and it stays**, and `rigs.ts` is not edited. **A constant no longer read by anything is deleted in the same commit**, because a constant nothing reads is the second spelling this step exists to remove; say in the note which ones went and which stayed and why.

**Sixth: the no-dead-row fence is added here.** A cross-cutting guard in its own file, named for the behaviour it guards, failing if a row of the default record has no reader in production code under `src/game` outside the record's own module. **`boundary.test.ts`'s own source-walking technique is the shape**; it is what makes "a row nothing reads is worse than no row" a rule the tree holds rather than a sentence in a prompt. **If your reading finds a truer mechanism, take it and say why in the note.**
...
4. **The new ADR the orchestrator filed for this step's shape**, and **ADR 0056 as amended**, for the purse and the quiet interval. Read both and name both in the note.
...
**(d) The purse.** The record's stage group at the grant, `Section`'s `purse` field removed, `SECTIONS`' six outside readers checked and reported.

**(e) The quiet interval's director reader**, off the run's record, with `QUIET_MIN_TICKS`' own derivation intact.
...
**(j) The progress note**, section **10**. Beyond the contract's list, say: every reader wired, by file and by row; the constants deleted and the constants kept with the reason for each; `Section`'s lost field and the six `SECTIONS` readers checked; the fence by title and the mechanism you gave it; the two probes whole; and the four constants and `GOLDEN` all named as read off your own tip.
...
- **`MOB_TYPES`' four `scorePayout` rows and the table's shape.** The table is eligible for the record and is not carried in this step.
- **Every existing invariant's meaning and severity, and every fault identity's wire number.**
- **`src/dev/rigs.ts`**, which reads `SCORE_BLEED_CAP` off the default for a starting condition and is correct as it stands.
...
**Five things are already known to be a stop:** any version constant or `GOLDEN` moving; an event count that moved on your twelve seeds; a reader this prompt does not name; a `SECTIONS` reader that reads `purse`; and a wiring that would need `MOB_TYPES`' own rows restated. **And four things are ruled rather than open:** the reader list, the purse leaving `Section`, the constants staying where something outside the core reads them, and no magnitude moving. **A measurement arguing any of the four should move is a finding for the note, never a row you move here.** **Green tests plus wrong observed behaviour means the test plan has a hole: pin the wrongness as a new red test first, never patch first.**
...
**Third: the refusal is precise and it is built, in these words.** A tape whose block names a row this build's record does not have, or **lacks one this build requires**, is **readable and reportable in the tape's own vocabulary and is not replayable here**. A tape whose names all match **replays under the tape's own values, whatever this build's defaults are**, which is the point of carrying values at all. **The refusal is a named outcome beside the roster's own**, not a divergence discovered at a checkpoint, which is ADR 0019's rule: a replay that cannot prove it is the original run reports nothing rather than reporting wrongly.
...
**(e) The resolve and the refusal.** One function asking whether this build implements what the block names, its refusal carrying the tape's own vocabulary, beside the roster's rather than folded into it. **A new `PlaybackOutcome` member if that is what the shape asks for**, and if that member needs a wire code, it is appended and moves nothing (ADR 0024's own rule, applied where the codes are append-only).

**(f) `runFromHeader`, `harnessHeader` and `headerFor`**, each building or reading the block rather than the four fields.
...
**(j) The progress note**, section **11**, **and section 2's ledger row**. Beyond the contract's list, say: the block's shape and the four fields it replaced; the bump's dated paragraph; the refusal's exact wording and the outcome name it answers with; the cost proved, with the measured tape's outcome quoted; the ladder tape verified and the gap named as closed, citing the handoff's open item 5 and step 5's note section 14; `rigOf` widened and slice 8's narrowness reversed with the reason; and the other three constants and `GOLDEN` named as read off your own tip.
...
`src/tape/wireCodes.ts` and the codec: the block encoded and decoded, round trip, at every row. `src/tape/tape.ts`: `TapeHeader` carrying one starting condition rather than four fields of one. `src/tape/playback.ts`: the resolve asking whether this build implements what the block names; the refusal when it does not; `runFromHeader` rebuilding the run from the block; a replay running under the tape's own values rather than the build's defaults. `src/dev/rigs.ts`: `rigOf` banding by the whole starting condition. `src/dev/harnessRun.ts` and `scripts/record-conditioned.ts`: a header written with the block.
...
9. *A tape recorded on a rig bands to that rig*, `rigOf` answering off the whole condition.
10. *Every rig row's starting condition is still unique* under the widened key.
