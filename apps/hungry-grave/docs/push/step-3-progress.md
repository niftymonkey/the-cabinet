...
## 20. The shared TypeScript config

Unrelated to ticket #98: a separate mechanical cleanup pass, folded into this note because it is the app's progress note and the coder-contract asked for it here. `apps/hungry-grave/tsconfig.json` now `extends` `../../tsconfig.base.json` instead of carrying its own copy of the compiler options, keeping only `resolveJsonModule` and `noUncheckedSideEffectImports` (the two options the base does not carry) and its own `include`. No behaviour changed anywhere; the existing suite is the proof.

**A blocking finding before any fix could start.** The base turns on `erasableSyntaxOnly`, which needs TypeScript 5.8 or newer, and hungry-grave's own `package.json` pinned `"typescript": "~5.7.3"` directly rather than `"catalog:"` the way its own `vitest` entry and every other app in the workspace already does. `tsc` refused the base config outright (`error TS5023: Unknown compiler option 'erasableSyntaxOnly'`) until that pin was changed to `catalog:`, which resolved to the workspace's 6.0.3 (already in the lockfile via housewarming) and needed `pnpm install --no-frozen-lockfile`. This is recorded rather than left silent because it is a real, three-version drift this app carried alone; every other devDependency in its `package.json` (eslint, vite, typescript-eslint) is still pinned locally rather than to the catalog, and none of those were touched here since nothing forced the question.

**A second, non-obvious finding after the compiler agreed to run: an ESLint config ambiguity, not a `tsc` error.** hungry-grave carries its own `eslint.config.mjs` separate from the repo root's `eslint.config.js`. Once `tsconfig.json` reaches the repo root through `extends`, typescript-eslint's project-service auto-discovery finds two candidate roots (the app and the repo root) for every file linted in one `eslint .` run and refuses to guess, which failed `pnpm lint` (part of `pnpm verify`) on all 327 of the app's own `.ts` files identically, and, in the other direction, on two root-level `.mjs`/`.js` files (`eslint.config.js` itself and `.claude/hooks/dispatch-contract.mjs`) that sit in neither tsconfig's `include`. Fixed by pinning `tsconfigRootDir: import.meta.dirname` explicitly in both `apps/hungry-grave/eslint.config.mjs` (its own TS-files block) and the repo root's `eslint.config.js` (widened onto its existing catch-all `js,mjs,ts,tsx` block, ahead of the ts/tsx-scoped `projectService` block). Two earlier attempts at the second file were wrong and reverted in place: widening the `projectService` block's glob to include js/mjs pulled orphan root files into the typed project graph and they were then rejected by `allowDefaultProject`; disabling `projectService` per-glob for js/mjs left the ambiguity error firing anyway, because the check runs before any per-file opt-out is consulted. Neither wrong attempt reached a commit. This is a repo-root file outside `apps/hungry-grave/`, touched because leaving `pnpm verify` red was not an option and the fix is one line mirroring a pattern already in that same file; it affects no other app's rules or behaviour, confirmed by `apps/housewarming`'s own lint and test runs staying green throughout.

**What `noUncheckedIndexedAccess` actually produced was an order of magnitude past "dozens."** Roughly 1,600 raw `tsc` error lines across just over 100 files, not the "dozens" the dispatch estimated. `verbatimModuleSyntax` and `forceConsistentCasingInFileNames` produced zero sites; the codebase already writes `import type` everywhere it needs to. `erasableSyntaxOnly` produced exactly two, both parameter-property constructors (`BelchButton.ts`, the prototype's `touch.ts`), fixed by declaring the field and assigning it in the constructor body. The ES2024 lib bump produced two sites of its own kind, unrelated to any of the four named checks: `new Blob([bytes], ...)` in `tapeExport.ts` and `RunsScreen.ts` stopped accepting a plain `Uint8Array` once its type widened to `Uint8Array<ArrayBufferLike>`, fixed by re-wrapping with `new Uint8Array(bytes)` at the call site, which copies the same bytes into a concrete `ArrayBuffer`-backed view.

**Every `noUncheckedIndexedAccess` site got a guard, a restructure, or the codebase's existing loud-failure idiom** (`if (x === undefined) throw new Error('<message>')`, already present in `src/prototypes/ugly-slice/game/spec.test.ts` and `src/dev/readings/timeToKill.ts` before this pass), lifted into one small unexported per-file helper wherever a file needed the same guard three or more times, never a new module, never exported, never shared across files. Two production restructures instead of guards: `src/game/stage/stage.ts`'s `spawnDueRows` now looks a due row up once instead of indexing `phase.rows[stage.firedRows]` twice, and `BackgroundRenderer.ts`'s `dressingFor` computes its phase index once before indexing. No non-null assertion and no `as` cast was introduced anywhere to reach green; every pre-existing `!` or `as` found in a touched file (`state.boss!`, `CURTAIN_ROWS[0]!`, one Graphics-instructions cast in `BossRenderer.test.ts`) was already there unchanged on both sides of the diff, confirmed by scanning every added diff line across the whole change. **Zero stop-and-report sites**: every error had a real fix, because every absence `noUncheckedIndexedAccess` flagged was either a genuinely reachable case the surrounding code already handled, or provably impossible by a nearby invariant (a modulo- or loop-bounded index, a mandatory regex capture group, a pool slot sized by construction, a parallel array).

**The dispatch's own "no test file edited" is false against the tree, and the intent behind it was followed instead.** Well over half of the roughly 1,600 sites lived inside `*.test.ts` files themselves (property-based invariant checks indexing arrays, `expect(only(events, 'x')[0].field)` chains, pooled fixtures), not in shared non-test helpers a production-side fix could have cleared for free. Sixty-four test files needed edits. None needed an `expect(...)` value, a test name, or a test count changed: every edit added a guard or restructure around an existing assertion so the same value it already checked would compile. Checked two ways: an automated count of `it(`/`it.todo(` declarations removed versus added came back matched for every one of the 64 files, and `pnpm vitest run` before the first commit and after the last one both report 138 test files, 1,824 passed, 10 expected fail, 2 todo (1,836 total).

**Whether the app should keep any option the base overrides: no.** Every compiler option the base sets was adopted as-is (including the `moduleResolution` casing, `"Bundler"` vs. the app's old `"bundler"`, which TypeScript treats identically). The only two kept in the app's own `tsconfig.json` are `resolveJsonModule` and `noUncheckedSideEffectImports`, both because the base does not carry them at all, exactly as the dispatch anticipated. Nothing else diverges.

**No browser build target is pinned.** `vite.config.ts`, `vite.headless.config.ts`, and `package.json` set no `build.target`, no `esbuild.target`, and no `browserslist`; Vite's own default stands. The `target: "ES2024"` bump is `tsc`'s type-checking floor only, not a change to what ships to a browser.

### Verification steps run

1. **`pnpm typecheck`**, green, zero errors.
2. **`pnpm vitest run`**, green: 138 test files, 1,824 passed, 10 expected fail, 2 todo (1,836 total), the same counts as the baseline captured before the first commit of this pass. 64 test files were edited; none needed an assertion changed (see above).
3. **`pnpm verify`** at the repo root, exit 0.
4. **The four fence files**, run explicitly and green, named by test title: `src/__tests__/boundary.test.ts` ("the rendering-import boundary", "the test-span fence", "the screen graph is declared in one place", "the engine accessor is out of the app", "the core has no import cycle"), `src/__tests__/lineAgnosticPolicies.test.ts` ("no weapon line walks the mob pool", "a policy names no weapon line", "no boss and no set piece names a weapon line", "a line's constants are declared in that line's own module", "only the offer draws from the drops stream"), `src/dev/__tests__/comparisonDeclared.test.ts` ("every reading declares what comparing it means"), `src/dev/__tests__/batchReadingDeclared.test.ts` ("every reading declares how a batch reduces it").
5. **`GOLDEN`, `WITNESS_VERSION`, `READINGS_VERSION`, `FORMAT_VERSION` unchanged**, proved by diff: `src/game/witness.ts`, `src/tape/wireCodes.ts`, and `src/dev/readingsVersion.ts` are byte-for-byte identical to their pre-change snapshots. `src/dev/digest.ts` changed (one guard added around a modulo-bounded script index), but `GOLDEN`'s own literal is untouched and `digest.test.ts` still passes against the same expected digest.
6. **Playing the deployed game, human, still open.**

Commits: `cd00e5af13` (the config change and its immediate fallout, plus the typescript catalog bump), `86634afdf1` (the eslint config fix), `60707ea831`, `15a00e1021`, `71ec22cfd7`, `b29d2361ef`, `ab85ca27e0`, `e1b04188c8` (one per large file group), `f8739917f5` (a trailing prettier-only reformat). CodeRabbit (`coderabbit review --agent --uncommitted`) ran twice: once over the first commit's files (one finding, declined: it proposed rejecting an empty path in `boundary.test.ts`'s `rootOf`, which is a real, exercised call (a test file directly under `src/__tests__` reports its root as `''`) and the change would have made that legitimate case throw instead of returning `''` as before, a behaviour change); once over the entire remaining diff before the six file-group commits, which found nothing.
