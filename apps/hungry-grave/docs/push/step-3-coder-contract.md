...
5. The test-name diff (verification step 5): capture the tree's listing and compare it with the slice 0 baseline, which is `apps/hungry-grave/local/step3/tests-baseline.txt` beside its `.json` sibling. Both files answer, because `scripts/test-names.ts` reads either shape; any test removed or renamed is reported. It matters most on the header slice, which touches fourteen files that build a `TapeHeader` literal. The two commands are in the list below, and the second exits non-zero the moment a name is gone.
...
- The test-name listing: `pnpm vitest list --json > <current>` and then `pnpm vite-node --config vite.headless.config.ts scripts/test-names.ts local/step3/tests-baseline.txt <current>`. The capture may carry the asset pipeline's log lines above the array; the command reads it anyway, so no capture is ever edited by hand.
