...
8. `apps/hungry-grave/docs/push/step-5-progress.md` **section 7 in full**, which is M1 building the kill input and is the nearest precedent to yours: the row shape, the payment site, the two kill paths it deliberately left unpaid as your gap to close, and the readings move it took. **Section 12 beside it** for M6's three readings, `fallenRungLedger`, `stripsLanded` and `bledRungMemory`, its held version and its per-seed table; **and section 14** for `createRun`'s trailing `startingScore`, `RIGS.ladder`, and why a `rig=ladder` batch cannot verify.
...
**(a) Verify the inputs.** `git log --oneline -25`, `git status --short`, the four constants read off the tree and reported, and your own test-name baseline into `local/step5/` under a name carrying `m7`. **Slices M1, M1-fix, M1-stage, M2 through M5, M5-fix and M6 must all be in the tree**; if any is missing, that is a stop. **Read all four constants yourself rather than off any line in this file**; the tip this block was reconciled against is `f756ea3f38`, and the test-name baseline there is 2,302 names.
...
- **A batch at your tip** on the configurations and seeds M1 and M6 used, with the score decomposed by input printed beside `run.score` and beside M6's three readings, **and compared per seed against section 12's table for those three and section 14's table for `damageTaken` and `run.score`: every cell of M6's three readings, `scoreBleeds`, `weaponStrips`, `linesStripped` and `seals` must reproduce, because this slice changes no rule they read.** **Say plainly that `run.score` and `tuning.damageTaken.scoreBled` are incomparable with every earlier batch's**, which is item (f)'s whole subject.
...
- **A conditioned tape measured to `outcome: 'verified'`**, at a rig that reaches a boss, because every one of your inputs is silent in a run that never leaves the Procession. Record it via `record-conditioned.ts`'s `rig=` flag (`rig=maxed`, or `rig=ladder score=0` as section 12 did); **`scripts/batch.ts rig=ladder` verifies nothing because no tape header carries a score, and that gap is not this slice's.**
...
- **The three ladder events, `scoreBled`, `weaponStripped` and `sealed`, stay three**, and `bleedScore` still takes the lesser of the standing score and `SCORE_BLEED_CAP` however many inputs fed it, and the cap row's value does not move.
