...
**The three close gates ran on 2026-09-17 over the step's whole diff at tip `8a833499d7`, and all three came back PASS WITH FINDINGS.** The orchestrator ruled every disposition and this section records them: what was folded, what became a ticket, and what is a note for a later reader. The findings ruled FOLD are in the close fold `88a3808617`, whose own account is section 13's last subsection, except ADR 0056's stale sentence, which this docs commit fixes. **A finding against something Mark ruled is filed for his read and never applied.**

**Tech gate.** FOLD, four fixes, all landed in the close fold: a zero quiet-interval minimum reached `capsFor` and `createRun` looped forever, now refused at the resolver and inherited at the tape edge; `GameScreen` and `FrameBudgetScreen` were both read as dressing at the default record's caps, and `GameScreen` now attaches at the run's own caps with its pin at `src/app/__tests__/GameScreen.test.ts` where the test-span fence put it, while `FrameBudgetScreen` is untouched because every run it makes carries no record; two stale comments, one in `run.ts` and one in `digest.ts`; and `BatchIdentity.tuning` is now `TuningRow[]`, so the report spells the record in the dotted rows the header, the comparison and the command line already use (the draft's ruling 2). TICKET #143: a run played under a non-default candidate is not marked `conditioned` and sits inside the default aggregates, which needs a readings bump and so is not a fix to take on the spot. NOTE, three things the record's own author cannot see: `countsWholeThings` and `PurseRow` both read the `Purse` name suffix, which makes that suffix a load-bearing convention; `sweep.ts` is `batch.ts` re-spelled and is the second copy, so `record-conditioned.ts` gaining `tuning=` would be the third and the extraction trigger; and `RunState.roster` and `RunState.conditions.roster` spell one fact twice.

**Vision gate.** PASS on the V1 goal: the tooling stays inside the harness, the one surface a player can reach is `?tuning=` repairing an unknown name to the default, the scope held, and no magnitude moved. FOLD, taken in this docs commit rather than in the code fold: ADR 0056's body still answered its fourth open item with "pinned to the build" under an amendment that had already reversed it, and it now points at the 2026-09-17 amendment, the budget travelling in the tape header as a row of the tuning record. NOTE for the ADR review, verbatim from the gate against Mark's bar of large, important, extremely tough to reverse and hard-won: 0064 clears; 0043's amendment clears; 0063 does not clear, being compliance with the code rules rather than a decision anyone would re-argue, and its durable fact already sits in 0043's amendment and in the glossary; 0062 does not clear, being a craft rule derived from three declines, whose record is `comparisonDeclared.test.ts` plus a line in `docs/lessons.md`; 0056's amendment half clears, the trigger-spent sentence needed and the implementation state not. **These are inputs to the branch's ADR review, which rules on all 64 with Mark; nothing is changed now.**

**Design gate.** NOTE, nothing a player meets moved: twelve seeds come back identical field for field across every slice. NOTE, the ten rows tune the economy and the purse and none of them reaches the vision's open recovery question: `HIT_SHRINK` and `CORPSES_TO_CEILING` are the first rows a designer wants, and then `crowdPurse` and `quietIntervalMinimumSeconds` as a pair, because neither of those two reads alone. NOTE, the Procession purse is not a lever: the section spends 9 to 26 of its 116, `liveFormationCeiling: 1` is the rule actually binding, and the Crowd is where a purse could bind and is unmeasured, on Risk of Rain 2's precedent of a director holding credits behind its own cap. NOTE, nine of the twelve first-sweep items in the handoff name no row this record can express, which the handoff's list now says at its end. TICKET #144: the score readout's digit budget assumes a kill unit no test pins.

**The three tickets the close created, all filed 2026-09-17.** **#143**, a run played under other numbers is averaged with the default runs. **#144**, the score readout's digit budget assumes a kill is worth a hundred. **#145**, the boundary check reads the words import and from in prose as imports, the finding sections 5, 11 and 13 each filed in turn.

**The close batch is slice 8's sweep at `ace50d5c29` re-run by the close fold**, `default` against `spendable` on the same eight seeds, `stage.processionPurse: 116 against 42` the first line under the pair and 336 of the 337 readings flat behind it. **No CodeRabbit run was taken over the step's range** (Mark, 2026-09-17); the per-commit runs each slice took are the reviews of record, and section 4 is their ledger.

**The close's tip is deployed.** `e7b45b2441`, the docs commit that records the close fold, is live on https://hungry-grave.vercel.app as deployment `dpl_8XdTU3cYQ85wTANXsyezyB7frg5S`, READY on production 2026-09-17. **What is left of the step is Mark's rundown and his merge call**, neither of which changes the tree.
