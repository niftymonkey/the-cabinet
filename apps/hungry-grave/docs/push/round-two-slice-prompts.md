...
## Slice J3: every eruption ring pushes what is inside it, and the press record gains a ring axis (#124)

Model: Opus, subagent type general-purpose. One coder, **two code commits and one docs commit**. Messages end in `(#124)`.

Round two slice J3 of The Hungry Grave (ticket #124): the belch draws three rings and pushes once. The second and third rings sweep a circle the crowd has walked back into and move nobody, so the picture promises three pushes and the field gets one. This slice makes each ring a real press of its own, and gives the press's record a ring axis so the next one of these is measured rather than watched.

**The standing rules are in `step-4-coder-contract.md`; read it first, and read the three overrides at the top of this file.** Everything below is what is specific to slice J3.

**Two things about this slice's own name and numbering, said here so nobody later reads either as a mistake.** It is **J3** on the same rule that made J2 J2: a suffix on this branch means a later pass on the same subject and never a dispatch position, and this is the fourth pass on the belch press, after J, J-fix and J2. And **its progress note section is 18**, not 16: **16 is slice L-fix and 17 is round two's own close**, both of which are being written while you read this. The note is appended to and never renumbered, so 18 is yours whether or not 16 and 17 are filled in when you arrive.

**Two code commits, and it is this round's third authorized departure from the contract's one-code-commit rule**, on the same reason slices H and J2 had. **The fold is certain rather than conditional**, for the reason ruling four gives, so the witness fold lands in its own commit declaring every new folded field, stamped in that commit, green on its own, with any rule test that needs the second commit's behaviour written as `test.todo` inside it. The second commit is the rest. **Splitting a finished body of work into two commits afterwards needs code edited or backed out, which is refused outright**, so build the fold commit whole and green and never split one later.

**Slice L and slice L-fix land before you. Verify each of these by name and any one missing is a stop and report:** `shoveNearbyTargets`, `pressOneBody`, `insideBurst`, `distanceFromGrave` and `fireBelch` in `src/game/belch.ts`, with `BELCH_BURST_RADIUS` still `FIELD_WIDTH / 2`, `BELCH_SHOVES` 3, `BELCH_SHOVE_THROW` 60 and `BELCH_SHOVE_SPACING` 30; `startShove`, `advanceShove`, `handOverImpulse`, `takeShoveTravel`, `impulseSpent`, `shoveInFlight` and `SHOVE_TICKS` 30 in `src/game/shove.ts`, with `Impulse` carrying `source`, `bodyId`, `shovesLeft`, `nextIn` and `spacing`; `ShoveCarrier` as the role a body and a corpse both fill; `stormTargets`, `shoveStormTarget` and `moveStormTarget` in `src/game/stormTargets.ts` with `SLOTS` sized `MOB_CAP + 2`; `PressedBody`, `PressRefusal` and `Belched` in `src/game/events.ts`; `eruptionFrontsAt`, `ERUPTION_TICKS` and `ERUPTION_REACH` in `src/app/screens/game/StormRenderer.ts`; and `WITNESS_VERSION` 9, `READINGS_VERSION` 6, `FORMAT_VERSION` 4 and `GOLDEN`'s checksum `1275540894`. **Read them off the tree rather than off this list**, because L-fix is landing while this prompt is written. Read round two progress note sections 11, 12, 15, 14 and 16 for what J, J-fix, J2, L and L-fix each landed.

**Why this slice exists, and it is a measurement off Mark's own tape rather than a sighting.** Mark played slice L's build on 2026-09-16 and said the bodies inside the eruption were not being pushed. **His ruling, in his words: "Everything within the eruption should be pushed on each erupt animation."** The tape is `hungry-grave-1999305952`, verified 373 of 373 at commit `dacbce3abd`, and a copy sits outside the repo at `/tmp/claude-1000/-home-mlo-dev-niftymonkey-the-cabinet/b77c279f-8a30-4610-9e67-47be0c1cc6e1/scratchpad/mark-1999305952.tape`. **It holds three presses and it says plainly what happened.**

**The belch itself is fair at the tick it lands.** On the two crowded presses every body inside the 270 radius was caught, with zero refusals inside the circle; the only refusal inside it anywhere was a boss, which is ADR 0007 working. **What is unfair is that there is only ever one tick.** `shoveNearbyTargets` walks `stormTargets` exactly once, at the press tick, and hands each caught body **one impulse carrying all three shoves**. Waves two and three never re-scan anything. Meanwhile `eruptionFrontsAt` redraws a front from 0 out to 270 three times over ninety ticks. **Wave one throws everyone out 180 units, the crowd walks back in over the next sixty ticks, and rings two and three sweep a refilled circle and move nobody in it.** Counting only bodies that were alive, entered, inside the burst radius and standing still at that wave's own tick: **wave one moved 89 percent of what was inside, wave two 47 percent, and wave three moved 0 of 11.**

**There is a second cause in the same tape and it is smaller and real.** `startShove` overwrites `shovesLeft` rather than taking the larger, so a bell toll landing at press plus thirty replaced a body's two remaining belch waves with the bell's single shove. **Body 16023 is the worked case: it stopped at 69 units where the press owed it 180.** A push that arrives is never allowed to take away a push already owed.

**And J2's instrument cannot see either of these, which is the third thing this slice fixes.** The per-body record reads the field once, at the press tick, and has no tick axis at all, so a press whose later rings move nobody and a press whose three rings all land read identically in every tape we hold.

**Five rulings shape this slice and none of them is yours to revisit.**

**First: each of the three waves re-scans the frame at its own tick, and gives what it finds the waves that remain.** The waves fire at the press tick, the press tick plus `BELCH_SHOVE_SPACING`, and the press tick plus twice it. **A body caught at wave one carries three shoves, a body that walks in by wave two carries two, a body that walks in by wave three carries one**, so the press always ends ninety ticks after it began and never grows a tail. **A body this press has already caught is not given a second impulse by a later wave of the same press; it keeps what it has**, because re-arming a shove in flight replaces it rather than following it, which is exactly the case `shove.ts`'s own spacing paragraph already warns about.

**The skip is "already caught by this press", read by body id off the press's own record, and it is never "already carrying a shove".** Both gates found the wider rule against Mark's own words on the same evidence: a toll's cone is on the field for most of a press, so a body standing under a ring while a bell shove carries it would be left alone by the belch, which the design gate measured at about two presses in five at any rung. **A bell-shoved body walking into the circle is thrown by the belch, at wave one as much as at wave two.** The press's caught record is folded state either way, per ruling four, so the exact rule costs nothing the cheap one would have saved. **`impulse.source` stays pure provenance and stays out of the fold**, which is what slice I defended (note section 9) and what this rule protects.

**The belch needs live per-press state in the run rather than a one-shot call**, and item (d) bounds the shape.

**Second: a new shove never shortens what a body already carries, and never re-times or re-throws what it still owes.** `startShove` takes **the larger** of what the impulse still owes and what the new push offers, and **the source stays whichever fired last**, which is slice H's attribution rule and does not move. Body 16023 of Mark's tape is the case: a bell shove arriving mid-press must not be able to cancel two belch waves the press already owed.

**When the carried count wins, `shovesLeft`, `spacing` and `nextIn` all stay exactly as they are, and only the shove in flight is replaced.** A re-arm of `nextIn` from the spacing reads as a reset, and a reset grows the press a tail for every toll that lands on it, which is a behaviour nobody ruled. **The invariant to pin is that `shovesLeft` greater than zero implies `nextIn` greater than zero**, so an owed wave always has a clock bringing it in.

**And the kept waves keep their own throw.** `startShove` overwrites `stepX` and `stepY` today, so a press's two remaining waves would travel at the toll's push rather than at the belch's sixty, which the design gate found and which is a magnitude change arriving by accident. **A newcomer's step governs only the shove in flight**; what the impulse still owes keeps the throw and the direction it was given. Finding the shape that expresses that is item (d)'s.

**This is a change to the one shove module and it reaches the bell as much as the belch** (design record R8).

**Third: the press's record gains a ring axis, and `READINGS_VERSION` moves 6 to 7.** One frame per wave, each carrying the bodies in the frame at that wave's own tick with their outcome and the gate that refused each one it did not move, and **the aggregate the batch prints gains per-wave moved counts** beside the `frameShares` and `misses` slice J2 gave it. That is a change of meaning on an existing reading, which is `readingsVersion.ts`'s own rule for when the version moves. **The record is the whole point of the slice and not a side effect of it**: without the ring axis, the next time a ring moves nobody there is nothing in any tape that says so.

**A body the press already caught is a third outcome and never an absence from the frame.** Both gates named this: drop it and a ring sweeping a crowd that is all in flight reads as an empty frame, which is the same unreadable zero slice J2 existed to kill, and `moved: false` with `refusal: null` becomes a silent state the type allows and nothing explains. **The outcome is `carried`, meaning already moving under this press.** It follows that **waves two and three count a carried body as caught by the press in their frame share, and count moved-this-wave separately**, and the note says that in those words so no later reader adds the two together.

**Version 7's note also names the repel arm**, per the design gate's deferred finding: a toll landing mid-press relabels the press's remaining travel as the bell's under the last-pusher rule, so `tuning.repel`'s bell arm now carries belch travel wherever a toll overlaps a press. That is attribution working as slice I ruled it and is not a thing to fix here, but a reading that does not say so is a reading that misleads.

**Fourth: `WITNESS_VERSION` moves 9 to 10, `READINGS_VERSION` moves 6 to 7, `FORMAT_VERSION` 4 holds, and `GOLDEN` re-pins exactly once. The re-pin is granted up front rather than yours to take.** Read this paragraph twice.

**Both moves are certain by construction and the first draft of this prompt was wrong to make either conditional.** The press is run state, a run field a replay must rebuild is folded (ADR 0019), and **an absent run field folds `ABSENT_CODE` rather than nothing**, which is exactly what `witness.ts` already does for the ending, the ring, the boss and the set piece. So a run with no press live folds a sentinel the old fold did not fold, the checksum moves on every tick of every scenario, and **a step that expected a green digest would have stopped by construction**. The tech gate found that before dispatch.

**The grant is the orchestrator's, dated 2026-09-16, under one-push mode, and it stands on ruling R9's own closing paragraph's reasoning**: the re-pin budget is the record's guard against a behavioural drift hiding inside a mechanical one, and never a bound Mark set; the trigger here is Mark's own sighting inside the round rather than a slice's convenience. **It is one re-pin, it reopens nothing, and no other slice is permitted one.**

**It is granted against a proof rather than a promise, exactly as slice J2's was.** Before you pin, show the diff has one cause: **run the canonical scenario with the press field held at rest, absent on all six hundred ticks, folded the old way, and show the old digest `1275540894` returns whole**, every field and the checksum. Then fold the press and pin what comes out. **Both digests go in the note, side by side.** **A digest diff with any other cause, a moved position, a moved count, a moved stream cursor, a moved level, is a stop and report and never a re-pin.** The isolation run **asserts the press is absent on all six hundred ticks rather than pinning it once and trusting it**, and it runs **at the fold commit's own tree** so nothing from the second commit is in it. That the scenario can reach nothing else of this slice is a fact worth printing beside the proof: `runScenario` passes `belch: false` on every tick and `levels.bell` is 0 throughout, so it presses no belch and rings no bell.

**The R9 amendment is yours to write, in your docs commit, and its text is in item (h) for you to copy in.** The new pin carries its own dated paragraph in `digest.ts`'s JSDoc naming every field that moved beside every field that held, the way slice J2's does, and its own entry in the note's section 3.

**Fifth: no belch number moves.** Not the radius, not the wave count, not the wave spacing, not the throw length, and the belch still does no damage to anything at all. **A measurement arguing any of them should move is a finding for the note**, which is the same standing rule slices J-fix, J2 and L each carried.

### Read first, in this order, before any edit

1. `docs/agents/feature-playbook.md` at the repo root. Read it and follow it.
2. `.claude/rules/code-core.md` and `.claude/rules/code-typescript.md`, plus `docs/agents/code-examples.md`, `docs/agents/lessons.md` and `apps/hungry-grave/docs/lessons.md`.
3. `apps/hungry-grave/docs/design/round-two-wall-belch.md`: **ruling R3 as superseded in full, which is where the three waves and the one front per wave were ruled together**, R11 for the reach and for the withdrawal of the sentence that three waves carry a body clear of it, R8 for the one shove module and why a change there reaches both callers, R1 for what folded per-body state costs, R9 for the `GOLDEN` and version rules you are working under, R10 for the impulse outliving its body, section 5 for what must not move, section 6 for the sentences already pinned, section 7 for the findings no slice acts on.
4. `apps/hungry-grave/docs/research/watched-pushback-duration.md` **sections 4 and 5**, which are where the thirty ticks, the three waves of sixty and the spacing all come from, and which is the record you cite if a measurement makes you want to move one of them. **You will not move one.**
5. `apps/hungry-grave/docs/adr/0008-the-belch-full-only-gas-everywhere-shove-nearby.md` as amended, which is what a press is and is not; `0019` for the fold and the refusal rule; `0015` for the golden digest and for why the tick count is the run; `0007` for why a boss is never moved; `0024` for the closed append-only fault identity list; `0056` for the refusal counters.
6. `apps/hungry-grave/docs/push/round-two-progress.md` **sections 12 and 15 in full**, which are the two slices whose work you are extending, plus section 2 for the version ledger, section 3 for how a `GOLDEN` entry is written, section 4 for what CodeRabbit has already found on this branch's shove work, and **sections 16 and 17 if they exist by the time you arrive**.
7. `apps/hungry-grave/docs/push/handoff.md`, the section **"## Findings filed for Mark's read, not applied"**, read by heading with `sed` and not whole.
8. `apps/hungry-grave/CONTEXT.md`, the entries **Burst, Belch, Mob, Corpse and Tape**, and **the Avoid lists before you name anything**. The new event this slice adds is named out of that register, per item (e).
9. The tree, whole where it is short and by function otherwise: `src/game/belch.ts` whole; `src/game/shove.ts` whole, **and its `Impulse` JSDoc's own sentence that no rule reads `source`, which item (d) turns on**; `src/game/stormTargets.ts`'s `stormTargets`, `shoveStormTarget`, `moveStormTarget`, `SLOTS` and `LIVE`; `src/game/step.ts`'s `step` whole, for the phase order and for where `fireBelch` sits in it, and `reportShovesLeftWithNoCarrier`; `src/game/run.ts`'s `RunState`, **and the `boss`, `offer`, `setPiece` and `director` fields as the four precedents for a single live record on the run**; `src/game/mobs.ts`'s `advanceMobs`, `damageMob`, `cullMobs` and `moveInsideBounds`; `src/game/witness.ts`'s `WITNESS_VERSION` JSDoc and its fold functions; `src/game/events.ts`'s `Belched`, `PressedBody`, `PressRefusal` and `MobShoved`; `src/dev/readings/belchCadence.ts` whole; `src/dev/readingsVersion.ts`; `src/dev/batchReport.ts` and `compareRuns.ts`; `src/dev/digest.ts`'s `runScenario` and `GOLDEN`; `src/app/screens/game/StormRenderer.ts`'s `eruptionFrontsAt`, `ERUPTION_TICKS` and `ERUPTION_REACH`, **to read and never to touch**; `src/app/screens/game/GameScreen.ts` and `src/app/screens/ReplayScreen.ts` where each calls `stormRenderer.erupt(run)` on a `belched`, and `src/app/sound.ts` where `belched` maps to `eruption`.
10. `local/jfix-equal-distance.ts` and `local/belch-play.ts`, the two uncommitted scripts earlier belch slices measured with. Both read their figures off the source, so both re-measure at your tip with no edit.

### The definition, in observable terms

After this slice: a press throws what is inside its circle **three times**, once as each ring goes out. A body standing outside the reach when the press lands, that walks into the circle before the second ring, is thrown by the second ring and by the third. A body caught by the first ring is carried by the press it was already given and is not re-armed by the rings behind it. The press still ends exactly ninety ticks after it began, still reaches exactly `FIELD_WIDTH / 2`, still throws each caught body sixty units a wave, and still takes health off nothing.

A body a bell toll is carrying when a ring reaches it is thrown by the ring, exactly as a walking body is. Being pushed by the other line is not a reason to be skipped.

And a bell toll landing on a body mid-press no longer shortens what the press owed it, nor re-times it, nor changes how far it travels. A body owed two more waves that a toll reaches keeps two waves, at the belch's own throw, on the belch's own clock, under whichever source pushed last.

And every press writes down **one frame per ring**: which bodies were in the frame at that ring's own tick, which of them it moved, which it was already carrying, and the gate that refused each one it did not. A batch can be asked what share of a press's own circle each of its three rings actually moved, which is the question Mark's tape had to be hand-measured to answer.

`FORMAT_VERSION` still reads 4, because no sim event is ever encoded into a tape at all and nothing new is recorded in a header. `READINGS_VERSION` reads 7. `WITNESS_VERSION` reads 10 with its move declared. **`GOLDEN` is re-pinned once, for the press's fold and for nothing else, against the isolation proof.** `pnpm verify` is green.

What a player meets: the three rings he can count each throw the crowd, instead of the first one throwing it and the other two sweeping over a crowd that has walked back in. **Ticket #124's done line is Mark's, unprompted, on the deploy after you**, and the orchestrator takes it.

### The work, in this order

**Four things read off the tree before you started, each of which would have cost you a rebuild.**

1. **`belched` is emitted on the press tick and two parts of the app key on it**, `stormRenderer.erupt(run)` in both `GameScreen` and `ReplayScreen`, and the `eruption` sound in `sound.ts`. `belchCadence`'s `tick` is that same press tick. **It stays exactly where it is**, which is why item (e) puts the later frames somewhere else rather than inside it.
2. **`impulse.source` is excluded from the witness fold precisely because no rule reads it.** `shove.ts`'s own `Impulse` JSDoc says so, `witness.test.ts`'s `EXCLUDED` holds it, and slice I's decline of CodeRabbit's contrary finding is the worked reasoning (note section 9). **Ruling one's skip is by body id off the press's own record and reads no provenance at all, so `source` stays excluded.** A shape that reads it is a stop rather than a choice.
3. **Distance is `Math.sqrt(x * x + y * y)` and never `Math.hypot`**, per `src/game/math.ts`'s own paragraph: `Math.sqrt` is exactly specified in the language and `Math.hypot` is implementation-approximated, and a determinism-critical core carries no approximated operation. There is deliberately no hypot wrapper and you do not add one.
4. **The frame is `MOB_CAP + 2`**, which is what `SLOTS` in `stormTargets.ts` is sized to: the mob pool plus the boss plus the set piece's source. Anything that allocates per frame reads that and never `MOB_CAP`. **A press now builds three frames instead of one**, so say in the note what that costs per press and read `stormTargets`' own retention rule before you keep anything out of it.

**(a) Verify the inputs**, per the stop above, then `git log --oneline -25`, `git status --short`, and your own test-name baseline into `local/round2/` under a name carrying the letters `j3`.

**(b) Measure before you change anything, off Mark's own tape.** Replay `mark-1999305952.tape` at your tip and print, **for each of the three presses and each of its three waves**: the bodies alive, entered, inside `BELCH_BURST_RADIUS` and not already carrying a belch shove at that wave's tick, and how many of them were moving under a belch shove on the tick after. **That is the before table**, and it reproduces because the build matches his; item (i) rules what the after table is and why it is a different kind of thing. It should give 89 percent, 47 percent and 0 of 11. **Copy the tape into `local/round2/` rather than reading it from the scratchpad path**, which is session-local and will not be there for the next reader, and say in the note where it came from. **A synthetic twelve-bodies-on-a-ring case is not the check and never becomes the check**: a new instrument is run over his tape against his own sighting before this slice is called done.

**(c) The tests first, red.** Write **the walks-in test first**: a body outside the reach at the press tick, standing inside it at the second wave's tick, is thrown by the second wave and carries exactly two shoves. Express it against the row rather than against a number typed in the test.

**(d) The shape, and this is where your real choices are.** Three properties bind whatever you build. **`src/game` stays dependency-free and pure**: no import of `src/app`, `src/dev` or `src/tape`, and the core's cycle guard keeps `KNOWN_CORE_CYCLES` empty. **`moveStormTarget`, `shoveStormTarget` and `pushable` stay the only way a body is moved and the only answer to whether it may be**, so a raw position write or a second reach test outside `belch.ts` is a stop. And **the shove stays one concept in one module** (R8), so the wave clock of a press does not become a second copy of the impulse's own.

- **Where the press lives.** A press that outlives its tick is run state, and `RunState` has four precedents for a single live record with no pool behind it: `boss`, `offer`, `setPiece` and `director`. **Follow them rather than inventing a fifth pattern**, and put the wave clock where `step` can advance it in a named phase whose place in the order you state and justify. **The belch's own phase runs before spawns and before every overlap for a reason `step`'s JSDoc gives**; a wave firing later in the tick than the press did is a behaviour difference you must not introduce by accident.
- **Two presses at once is reachable and the answer is already known, so build for it rather than measuring your way to it.** `RESERVOIR_CAPACITY` is `FEAST_PAYOUT` in `tuning.ts`, so **one fresh feast fills the reservoir whole** and a second press inside ninety ticks needs no unusual swallow rate at all. **The ruling: the newer press replaces the live one whole and its own three waves run from its own tick**, and a body the older press had caught is not protected by that record any more, so the new press may catch it. Test 15 stays live and pins it.
- **The skip rule, ruled rather than open.** Ruling one settles it: **skip a body this press has already caught, by id off the press's own record, and never a body merely carrying a shove.** The wider rule leaves a toll's cone bodies standing under a ring, which is Mark's sighting arriving again by a different route. The caught record is folded with the press, so the exact rule costs nothing.
- **The larger-of rule and the two parts it does not answer on its own**, both ruled in ruling two and both a real shape question here. **When the carried count wins, `shovesLeft`, `spacing` and `nextIn` all stay untouched and only the shove in flight is replaced**, because re-arming `nextIn` reads as a reset and a reset grows the press a tail for every toll that lands on it. **And the waves the impulse still owes keep their own throw and their own direction**, which `startShove` cannot express today because it writes `stepX` and `stepY` for the whole impulse: what a newcomer's step governs is the shove in flight and nothing behind it. **Find the shape that says that once**, write it as a constraint comment beside the code, and pin **the invariant that `shovesLeft` greater than zero implies `nextIn` greater than zero** as its own test.
- **A corpse is never re-caught, and that is correct.** Since slice J2 a corpse can be the carrier when a later wave's tick arrives, and `stormTargets` holds mobs, the boss and the set piece's source and no corpses at all, so a later ring never sees one. **Say that in the note as a thing checked rather than leave it to be rediscovered.** A ring re-throwing corpses is not this slice's work and is not a thing to build.

**(e) The record's ring axis, and the event it rides on.** **`belched` keeps its place on the press tick and keeps `cancelled` and `shoved`**, because the eruption, the sound and `belchCadence`'s tick all key on it. **What it cannot do is carry a frame taken sixty ticks after it was emitted**, so the per-body frames move off it onto **a new sim event, one per wave of the press, including the first**, emitted on that wave's own tick and carrying the press's own tick, the wave's ordinal, and the frame. **All three waves report through one shape**, so no reader special-cases the first; that symmetry is the reason the wave-one frame moves off `belched` rather than staying on it. **`shoved` keeps its exact meaning, the count of bodies the press moved on the tick it landed, and a test pins it equal to the wave-one event's own moved count.**

**Say plainly, in the note and in both JSDocs, that `bodies` leaves `belched`**, which is a field slice J2 added one slice ago, and that **`belchCadence` now joins the wave-one event to its press by the press tick** rather than reading the frame off `belched` directly. A reader who does not know that reads a `Belched` with no frame in it as a regression.

- **The event's name is research rather than a pick**, out of `CONTEXT.md`'s own register and checked against its Avoid lists, the way slice L chose its mob type's name. `pressWave`, `eruptionWave` and `belchWave` are candidates and not a shortlist. **Say what you chose and what you set it against.** It is not Mark's to pick and it is not a question for him.
- **The four refusal reasons do not change and you invent no fifth.** `notEntered`, `outOfReach`, `noDirection` and `notPushable` are the gates as `pressOneBody` runs them, and **there is still deliberately no already-dead reason**, for the reason `PressRefusal`'s own JSDoc gives.
- **A body this press already caught is a third outcome on the entry, `carried`, and never an absence from the frame.** Ruling three settles it and both gates found the alternative. Drop the entry and a ring over a crowd that is all in flight reads as an empty frame, which is the unreadable zero slice J2 existed to kill; write it as `moved: false` with `refusal: null` and the type now allows a state nothing explains. **The outcome union is the shape to change**, so every entry says one of moved, carried, or refused with its gate, and a `PressedBody` that is none of the three cannot be constructed. **Waves two and three count a carried body as caught by the press in their frame share, and count moved-this-wave separately**, and the reading's JSDoc says that in those words so nobody adds the two together.
- **The alternative that was considered and refused**, said here so nobody re-proposes it: deferring the `belched` event to the last wave so one event carries all three frames. It would move the eruption, the sound and the cadence reading's own tick a second late, which is the button lying at the only moment it matters.

**(f) The readings, and the version move.** The per-wave frames are declared as a reading in `batchReport.ts` and `compareRuns.ts` the way slice J2 declared the press record, reduced to something a batch prints: **per-wave moved counts and per-wave frame shares beside the misses the cadence already carries**. `READINGS_VERSION` moves 6 to 7 with version 7's own note naming what changed meaning, and **naming plainly that a press's shove counts before and after this slice are not subtractable**, because a press now starts up to three times as many shoves as it did. **Every batch recorded at slice L's tip is incomparable with every batch recorded after you on the belch arm.**

**Version 7's note also names the repel arm**, per ruling three and the design gate's deferred finding: under the last-pusher rule a toll landing mid-press relabels the press's remaining travel as the bell's, so **`tuning.repel`'s bell arm now carries belch travel wherever a toll overlaps a press**, and it does so more often after this slice because a press now owes waves to more bodies for longer. That is slice I's attribution rule working rather than breaking, and **it is named in the reading rather than fixed here**.

**(g) The witness, in its own commit.** The press is run state a replay must rebuild, so every field of it is folded, declared in `WITNESS_VERSION`'s JSDoc as a dated paragraph naming each field and why it is not excluded, and stamped in the same commit. **`WITNESS_VERSION` moves 9 to 10.** The caught record ruling one reads is part of it. **`witness.test.ts`'s `EXCLUDED` list is the precedent for anything that is pure provenance**, and `impulse.source` stays on it. The cost paragraph says plainly that every tape recorded before this commit is refused at the decode, and that it is the sixth time this step has made saved tapes a dead baseline. **A run with no press live folds `ABSENT_CODE`**, which is what `witness.ts` already does for the ending, the ring, the boss and the set piece, and which is the whole reason `GOLDEN` moves.

**(h) `GOLDEN`, re-pinned once, proved to have one cause, and the R9 amendment that grants it.** Run `digest.test.ts`. **It should be red**, because an absent press folds a sentinel on every tick; **a green digest is a stop and report**, since it means the press is not folded and a replay cannot rebuild it. **Then do the isolation run before you pin anything**, per ruling four: the canonical scenario with the press asserted absent on all six hundred ticks and folded the old way, returning `1275540894` whole, every field and the checksum, **run at the fold commit's own tree**. Only then pin the new checksum, with the dated paragraph in `digest.ts`'s JSDoc naming every field that moved beside every field that held, the section 3 entry, and both digests in the note. **A diff with any other cause is a stop and never a re-pin.**

**The R9 amendment is yours and it lands in your docs commit**, as a second dated closing paragraph on ruling R9 in the design record's section 2, in the same triple form slice J2's grant used, with section 5 pointing at it. **Do not edit the record before your code is green**, and read it fresh when you get there rather than trusting this text. It says, in substance:

> **2026-09-16, a second grant, to slice J3 alone.** What stood: a re-pin is granted against a proof and never a promise, one slice at a time, and the budget exists to catch a behavioural drift hiding inside a mechanical one. What it replaces: the reading that round two's two permits were a bound, when what they were is this record's own guard; the trigger here is Mark's sighting inside the round rather than a slice's convenience, and it is the orchestrator's under one-push mode. What it could not have known: that every ring of a press pushing what stands in it needs the press to be run state, and that an absent run field folds `ABSENT_CODE` (ADR 0019, `witness.ts`), so `WITNESS_VERSION` 10 and a `GOLDEN` move are certain by construction and not a choice any shape could avoid. **It is one re-pin, for the press's fold alone, proved by the isolation run, and no other slice is permitted one.**

**(i) The measurements this slice owes.**

- **The three presses' per-wave table, before and after, and the route matters because a replay of his tape at your tip is not a replay.** **Before: off Mark's tape at the current tip**, which reproduces because the build matches. **After: a readback of his commands against a changed field**, since the field diverges the moment your first press behaves differently and `playback.ts` sets its `exhausted` flag at the first divergent checkpoint. **Name that route in the note and label the two tables as what they are**: one is his run, the other is his inputs over a different world, and the presses in it are not his presses. **The pass line is therefore the per-wave property on whatever presses do fire**, not a match against his three: at every wave's tick of every press, every alive entered pushable body inside the burst radius is moving under a belch shove or has just been given one, and the wave's own record lists it as moved or carried. **Mark's own play on the deploy after you stays the done line**, and this is the evidence that it is worth deploying.
- **What a press now costs the field**, measured: shoves started per press before and after, and the share of a press's own circle each of the three rings moves. **A ring that still moves nobody is a finding and the first thing to explain.**
- **Body 16023's case re-run**, or the nearest equivalent: a body a toll reaches mid-press ending with both owed waves still to come, at the belch's own throw and on the belch's own clock. **Read it as the waves received rather than as a typed total distance**, because the exact figure depends on where the bounds and the scroll put the body.
- **A batch at your tip** under `steady-far` and `loose-far` on slice I's seeds, **900 to 905 under each hand**, with the belch arm, the cadence and the new per-wave counts printed. **Compare the belch distances against note section 12's table and say plainly that they are not subtractable any more.** All three `state.refusals` counters printed, `damage.belch` and `fatalBlows.belch` at zero on all twelve, `belchWorthIt` printed and left alone.
- **Replay determinism at your tip with a press mid-waves at a checkpoint.** A press spans ninety ticks against a sixty-tick checkpoint grid, so it always crosses one by arithmetic, which is slice J's own finding. Two plays of one seed under `shaky-short`, and a seed with a belch in it played twice under `steady-far`.
- **A pre-fold tape refused by its witness version rather than diverging**, once, off your own fold commit. Slice H's own verification and the version move's whole point.
- **The isolation run's two digests**, printed side by side: `1275540894` returned whole under the old fold with the press asserted absent, and the new pin beside it.
- **A hand-recorded tape at your tip**, against the built app through `vite preview`, driven with `playwright-cli`, measured to `outcome: 'verified'`, with all three `state.refusals` counters printed. **A blind driver cannot fill the reservoir and a hand tape will not carry a belch**; that is measured and written down three times already (note sections 11, 12 and 15), so do not spend runs chasing one. Take the belch's own behaviour off Mark's tape and off recorded ones, and say which route you used.
- **A rendered check, and read the screenshots.** A press on a crowd, replayed, with **the crowd visibly thrown by the second and third rings and not only the first**. **Play a run, end it, and play another.** Read note sections 11, 12 and 15 for what the earlier rendered checks could and could not catch before you spend the time, including the driver's 74 to 120 ticks between consecutive frames, **which is wider than one wave's own thirty**, and say plainly what you could and could not see.

**(j) CodeRabbit CLI, one iteration, then the code commits.** The fold commit first, something in the shape of `feat(hungry-grave): the press is state the run carries and the witness folds it (#124)`, then the rest, something in the shape of `feat(hungry-grave): every ring of a press throws what stands inside it and the record gains a ring axis (#124)`.

**(k) The progress note**, section **18**. Beyond the contract's list, say: the before table off Mark's tape and the after table off the readback, each labelled with its route and the readback's divergence named; the shape you chose, where the press lives, and what you set it against; the skip by id and `impulse.source` named as still excluded; the larger-of rule with the kept count, spacing, clock and throw each named, the `shovesLeft` implies `nextIn` invariant, and body 16023's case; two presses inside ninety ticks reachable off `RESERVOIR_CAPACITY` equalling `FEAST_PAYOUT`, and what a second press does to the first; the new event's name, what you set it against and its Avoid check; `bodies` leaving `belched` and how `belchCadence` now joins the wave-one event; the `carried` outcome and how the frame shares count it; what the frames cost per press; `READINGS_VERSION` 7 with what the move costs on the belch arm and the repel arm's relabelling named; `WITNESS_VERSION` 10 with the cost paragraph's sixth dead baseline; `GOLDEN` re-pinned with both digests side by side, the dated paragraph's location and the R9 amendment's wording; and `FORMAT_VERSION` 4 named as held.

**(l) Stop and report.** Under 300 words. **Do not start anything after this slice; the gates, the review, the batch and the deploy are the orchestrator's.**

### What must not move, and a move is a stop

- **`GOLDEN` moves exactly once and only for the press's fold.** The re-pin is granted in ruling four, the R9 amendment in item (h) is what records it, and the isolation run is what earns it. **A second move, or a first with any cause other than the fold, is a stop and report.**
- **`FORMAT_VERSION` 4.** No sim event is ever encoded into a tape and nothing new is recorded in a header, so a new event costs no bytes.
- **The fences**, all six by title, plus the core's cycle guard with `KNOWN_CORE_CYCLES` empty, plus *src/game/belch.ts reaches what it can hit through the seam* and *a policy names no weapon line*.
- **`BELCH_SHOVES` 3, `BELCH_SHOVE_THROW` 60, `BELCH_SHOVE_SPACING` 30 and `BELCH_BURST_RADIUS` at `FIELD_WIDTH / 2`.** Mark's own picks and rulings R3 and R11. The fifth ruling above. A measurement arguing any of them should move is a finding for the note.
- **`SHOVE_TICKS` 30, the linear decay, `firstStepOf`, and `travelled` accumulating the shove's own step and never the scroll.** Slices H and H2's.
- **The eruption and everything the renderer draws.** `eruptionFrontsAt`, `ERUPTION_REACH`, `ERUPTION_TICKS`, the stroke, the drift and the anchors are slice J-fix's. **The picture is already right and it is the push that is wrong**, which is the whole shape of this slice. R12's curve and body are demoted to tuning-step inputs and are not yours.
- **The gas**, in every part, and **the belch's no-damage rule**. A measurement showing the belch is weak is never a reason to give it damage back.
- **The bell's own arithmetic**, its two reaches, its `toll.struck` and its rung table. The larger-of rule reaches it because there is one shove module, which is R8 working as intended and is not a second thing to build.
- **`moveStormTarget`, `shoveStormTarget` and `pushable`.** A raw position write from anywhere is a stop, and a boss and a set piece's source are still never moved.
- **`stormTargets`' retention rule.** The list is answered for the moment it is asked for and no caller retains it past its own pass. **A press that holds a target across sixty ticks would be a reading of whatever now stands in that slot**, which is the exact trap that module's own header names; carry ids and ask again.
- **`scrollField` and the phase order in `src/game/step.ts`**, except for the one named phase your wave clock needs, whose position you state and justify.
- **Every cap, every fault identity, `STREAM_SALTS` and `STREAM_ORDER`.** The fault list is closed and append-only (ADR 0024). **A press draws from no stream and you add none.**
- **The harness's own rows**, `belchWorthIt` above all.
- **The four `PressRefusal` reasons**, and the deliberate absence of an already-dead one. **`carried` is an outcome and never a fifth refusal.**
- **`impulse.source` stays out of the witness fold.** No rule reads it, ruling one's skip reads the press's caught record instead, and slice I's declined finding is the reasoning. **A rule that reads it is a stop.**
- **You file no ADR and amend none.** ADR 0008 already rules what a belch does and ADR 0019 already rules that folded state gets a version; **applying a rule is not amending it.** `CONTEXT.md`, a module JSDoc and the design record are not ADRs, so **the R9 amendment in item (h) is not an exception to this line**, and it is the only edit you make to the record.

### Seams under test

`src/game/belch.ts`: a press whose three waves each re-scan the frame at their own tick and each hand what they find the waves that remain, and a record written per wave. `src/game/shove.ts`: `startShove` taking the larger of what is owed and what is offered, leaving the kept count, its clock and its throw alone and replacing only the shove in flight. `src/game/step.ts`: the press's wave clock advanced in a named phase whose place in the order is stated. `src/game/run.ts`: the live press, held the way the boss, the offer and the set piece are held. `src/game/stormTargets.ts`: read and not re-authored, with the frame asked for afresh at each wave. `src/game/witness.ts`: the fold over the press, and the sentinel an absent one folds.

### Module boundaries

**Nothing is created, deleted, merged or split**, unless the press's own record earns a file of its own, in which case it is a concept module named for the press and every export serves it, and **you say in the note why the concept did not fit in `belch.ts`**. No import direction changes: `belch.ts` still reaches the field through `stormTargets.ts` and never into `src/app`, `src/dev` or `src/tape`; `shove.ts` still imports nothing but the math helpers and the types; `src/dev` still reads events and never writes the run. **The renderer derives `ERUPTION_TICKS` from the belch's rows and the sim must not learn that in the other direction**: the sim's wave clock is derived from `BELCH_SHOVES` and `BELCH_SHOVE_SPACING` at its own end, and neither module imports the other. No new library enters.

### The planned test list

1. *A body that walks into the reach after the press lands is thrown by the next ring.* **Write it first, red first**, expressed against the row.
2. *A body this press already caught is not re-armed by the rings behind it.* Ruling one's second half.
3. *A body a bell toll is carrying when a ring reaches it is thrown by the ring.* **The separating test, and the reason the skip is by id and never by whether a shove is in flight.** Write it beside test 1.
4. *A body caught by the second ring carries two shoves, and one caught by the third carries one.* The press ends where it always ended.
5. *A press still ends exactly ninety ticks after it began, whatever walked in.* No tail.
6. *A press whose circle is empty at the first ring and full at the third still throws the crowd.* Mark's own case, turned into a test.
7. *A body a toll reaches mid-press still receives both waves the press owed it.* Ruling two, body 16023. **Read the waves received, never a distance typed in the test.**
8. *A wave the press still owes travels the belch's own throw, whatever pushed the body last.* The kept waves keep their throw; a newcomer's step governs only the shove in flight.
9. *An impulse that still owes a shove always has a clock bringing it in.* The `shovesLeft` implies `nextIn` invariant, and the tail a re-armed clock would grow.
10. *A bell toll and a belch press on the same body attribute the travel to whichever pushed last.* Slice I's attribution rule, held.
11. *Each ring writes its own frame, with every body in the frame at that ring's own tick.*
12. *A body refused by each of the four gates is recorded with its own reason, on whichever ring refused it.*
13. *A body the press already caught is in every later ring's frame, as carried.* Never an absence, and no ring over a flying crowd reads empty.
14. *The press's first-ring moved count is the same number `shoved` has always been.*
15. *A boss and a set piece's source are recorded by every ring and are still never moved.*
16. *A press still takes no health off anything, boss included.* Held.
17. *A corpse carrying a shove is never caught by a later ring.* The thing checked rather than rediscovered.
18. *A second press landing while one still owes waves replaces it, and its own rings run from its own tick.* Reachable on one feast, per item (d).
19. *A replay with a press mid-waves at a checkpoint rebuilds identically.*
20. *A tape recorded before the witness moved is refused by its version rather than diverging.*
21. **The six fences**, green, each by title, plus the core's cycle guard with `KNOWN_CORE_CYCLES` still empty.
22. **The golden digest**, re-pinned for the press's fold and proved by the isolation run to have no other cause.

**What this slice is expected to turn red.** `belch.test.ts` throughout, `shove.test.ts` over the larger-of rule, `step.test.ts` and `advance.test.ts` over the event stream and the phase order, `stormTargets.test.ts`, `witness.test.ts`, `digest.test.ts`, `belchCadence.test.ts` and `repel.test.ts` wherever a count or a distance is asserted, `measure.test.ts`'s rich fixture, `sound.test.ts` and `quietOnTheHappyPath.test.ts` over the new event, `bot.test.ts` where it counts `belched`, and `harnessPolicy.test.ts`'s measured per-seed baselines, which are **re-measured with the reason beside each and never re-pinned blind**. **A realistic count is 15 to 25 files**, against slice J2's 20 to 30 and slice H's 18. A diff much smaller than that is a reason to check what you have not reached.

### Verification steps, with actors

1. **Agent.** The before table off Mark's tape and the after table off the readback, each labelled with its route, read against the per-wave pass line in item (i).
2. **Agent.** Shoves started per press before and after, and the share of the circle each ring moves.
3. **Agent.** The mid-press toll case, read as the waves the body still received and at the belch's own throw.
4. **Agent.** `WITNESS_VERSION` 10 and `READINGS_VERSION` 7 each stated with what the move costs, and `FORMAT_VERSION` 4 held.
5. **Agent.** The isolation run: the canonical scenario with the press absent on all six hundred ticks under the old fold returning `1275540894` whole, then the new pin beside it, with the R9 amendment's wording quoted.
6. **Agent.** A pre-fold tape refused by its version rather than diverging.
7. **Agent.** Replay determinism with a press mid-waves at a checkpoint, on two seeds.
8. **Agent.** A hand tape verified with the three refusal counters, and the route used to measure a belch said plainly.
9. **Agent.** A batch at this tip on slice I's seeds, with the belch arm and the per-wave counts printed, and the incomparability stated on both the belch arm and the repel arm.
10. **Agent.** A rendered check across two runs with the screenshots read, saying in words whether the second and third rings visibly throw the crowd.
11. **Human (Mark), and none of these blocks you.** Whether everything inside the eruption now gets pushed on each ring, which is his own ruling answered in his own words. **The deploy that puts it in front of him is the orchestrator's, straight after your report.**

### State of the branch

- **Read all four versions off the tree and not off this list, because slice L-fix is landing while this prompt is written.** At the tip this was written against: **`WITNESS_VERSION` 9** (`src/game/witness.ts`), **`READINGS_VERSION` 6** (`src/dev/readingsVersion.ts`), **`FORMAT_VERSION` 4** (`src/tape/wireCodes.ts`) and **`GOLDEN`'s checksum `1275540894`** (`src/dev/digest.ts`). Slice H moved the witness to 8 and slice J2 to 9; slice I moved the readings to 5 and slice J2 to 6. **A version further on than this line is a fact for your note rather than a stop; a version behind it is a stop.**
- **Round two's own budget of two is spent**, slice H's taken and slice J's forfeit, and the one further grant beyond it was used by slice J2 to pin `1275540894`. **Yours is a second grant beyond that, for the press's fold alone, granted up front in ruling four and recorded by the R9 amendment you write in item (h).** Slice L was permitted none and took none.
- Read round two progress note sections 8 through 18 for the test counts and the test-name lists, **and confirm the count off the tree rather than off any line in the note**, because L and L-fix both moved it.
- The headless browser runs at 3 to 21 FPS under SwiftShader and puts consecutive screenshots 74 to 120 ticks apart, which is wider than one wave. That is the harness and not the build.
- Mark's tape is verified 373 of 373 at commit `dacbce3abd`, which is slice L's code commit.

### The stuck rule

**Five things are already known to be a stop:** any of the named inputs missing; a second `GOLDEN` move, or a first with any cause other than the press's fold; a green digest where the fold should have moved it; a `FORMAT_VERSION` move; and a fence, a cap or a fault identity moving. **And five things are ruled rather than open:** every ring re-scans and pushes, which is Mark's own words and is not a thing to measure your way out of; the skip is by id off the press's record and never by whether a shove is in flight; a new shove never shortens, re-times or re-throws what is already owed; the belch's four rows and its no-damage rule do not move; and the eruption's picture is already right and is not touched. **Green tests plus a ring that still moves nobody means the test plan has a hole: pin the wrongness as a new red test first, never patch first.**

### What is not your job

- **The Wall**, in every part, and slice L-fix's work on it. `waves.ts`, `formations.ts` and the mob table are opened only to read.
- **Step 5**, in every part. Nothing under `src/app/screens/game`'s HUD is in this slice and no portrait work is yours.
- **The bell's own shove count.** `sweepToll` pushing before it damages nearly doubled it, which slice J2 filed as a finding in note section 15. The larger-of rule touches the shove module the bell shares; **the bell's rung table, its reaches and its own count do not move.**
- **The four findings slice J2's note section 15 filed rather than acted on**: a corpse a push throws being food the hand never gets back, `StormTarget.killableOutright` having no production reader and a stale JSDoc, the bell's shove count, and the fourth exit that ends a flight when a corpse is culled or taken under. None is yours.
- **Softening the reach's hard edge, or drawing a second circle.** Filed as open by slice J2 and still open.
- **R12's curve and body**, which Mark demoted to tuning-step inputs on 2026-09-16 with *"I want a V1. Please help me get there."*
- **Retuning anything because a press now throws more.** That is a measurement and a finding for the tuning step.
- **Territory's rungs**, #125, the offer bubble, #122, and the resisted push and the boss's per-toll tell, #131 and #132. None is round two's.
- **The gates, the review on the tip, the orchestrator's batch and the deploy.**
- **The record's section 7 findings and the handoff's filed findings**, none of which any slice acts on.
