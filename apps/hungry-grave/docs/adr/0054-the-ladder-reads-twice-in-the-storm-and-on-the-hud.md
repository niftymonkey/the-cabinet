...
Every line's level is carried by that line's own projectiles, so the player reads the whole ladder by looking at the storm they are already looking at, and the slim HUD confirms it with the score and a row of level pips per line. A rung gained lights on both channels. A rung lost shows on the field as that line's projectiles blowing up and on the HUD as a pip going dark, so the player sees which line paid.
