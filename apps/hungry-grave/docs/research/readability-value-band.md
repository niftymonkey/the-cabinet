...
**The hit announces by subtraction, settled 2026-08-20 and amended with the boundary work on 2026-08-22.** The dim is a brief one on its own layer, directly beneath mob fire and above the grave's rim, so it is the field between them that darkens and both channels that must stay readable are spared. Sparing the rim is not a refinement of the rule, it is the rule: the grave is the health bar (ADR 0003), so the rim is the second channel, and dimming it would occlude the announcing channel at the exact tick it changes, leaving the player re-reading their own size on a field they have just been knocked around on.
...
**The boundary is a readout and not scenery, and that is why it carries a floor of its own.** The band gives mob fire a floor and everything else a ceiling, and asks of nothing else that it be visible at all; the stroke is the exception, because it is the whole statement of where the world ends, so it holds a contrast floor against the ground it sits on. The release named above was priced and is not available on this palette, where APCA clips to Lc 0.00 for every pair of large fills in the near-black region, so the floor stands until the field reads as bounded by structure, and the day it is released the reason is written down here.
