...
Written for a title-screen toggle: one line as the label, one sentence under it. The tape carries an author name field and an input-device field and nothing else, so the sentence can afford to be exact.

**A. Payload-first.**
> Send my runs to the developer
> Each finished run uploads its tape: the dice it rolled and the steering you gave it, plus a name if you type one. Nothing else leaves your device.

**B. Benefit-first, the 0 A.D. shape.**
> Help tune The Hungry Grave
> You can send each finished run to the designer so the game can be tuned by watching real runs. A run is a seed and your inputs, no account and no personal data.
