...
- **Any displacement of the grave re-anchors the drag, and nobody has decided that.** `TouchSteer`'s anti-windup rule fires whenever the grave is not where the command put it, for any reason at all, so when mob contact arrives the grave stays where it was knocked to rather than snapping back under the finger. That is almost certainly the wanted behaviour and it is a real feel decision that arrived as a side effect of the clamp fix. It lives in a `touch.ts` comment and nowhere else, which is how the product vision gate found it. Trigger: dispatch 4, when anything can displace the grave.
- **`#/digest` is dev surface on the public build with no stated exit.** It joins `?size=`, the corner readouts and `#/prototypes`, and unlike the prototype list nothing owns retiring it. It is cheap to keep and it is the only cross-engine check the game can run on a phone. Trigger: #38, with the corner readouts.
- **There is no render interpolation, and that changes how a high-refresh desktop read is interpreted.** The sim is a fixed 60 Hz and the grave draws wherever the last tick left it, so above 60 Hz it holds for two or three frames and jumps. Mark's phone read is unaffected: WebKit bug 272165 and its open successor 280183 record that iPhone Pro Safari does not run `requestAnimationFrame` at 120 Hz, and the "Prefer Page Rendering Updates near 60fps" default holds it near 60. The desktop keyboard read is the one at risk, where judder from the absent interpolation reads as steppy input with the speed slider right there to be mistuned against it. `clock.remainderMs` is already the alpha if this is ever built. Raised by the tech architecture gate on the implementation and deliberately not built here.
- **`GRAVE_RIM_STROKE`'s 2 CSS pixel floor is a phone claim held in a field-unit constant.** `main.ts` sets `minWidth: 540`, so a 320-wide viewport renders the 3-unit rim at 1.78 CSS pixels, under the floor its own derivation states. The rim cannot simply widen without thought, because an inward stroke eats the mouth, so the real trade is rim legibility against mouth width at `SIZE_FLOOR` rather than anything ADR 0003 forbids. Low stakes: the whole floor-size grave is 10.7 CSS pixels wide at that viewport.
- **`?size=` writes `run.grave.size` from `src/app`, around the rules layer.** `sizeFromUrl` enforces `SIZE_FLOOR` and `SIZE_CEILING` in the app layer, so the sim's own hard bounds are defended by a URL parser. This plan specified that shape and it is small, but this bullet's own first item makes `hitGrave` the single entry point for every size change, and this is the one write that goes around it. A starting size in `createRun`'s signature puts the clamp back in the rules layer. Trigger: dispatch 4, with `hitGrave`.
