...
**Ruled 2026-09-09, and this record is where the three rulings stand.** Growth over the run is authored per section as stepped values, never lerped between a start figure and an end figure. The rate holds at zero until the section's teaching rows have fired and steps up from behind them, because a repeating row running from the section's first second would bury the run's first swallow and its first mob fire under arrivals. And a standing row ends at the next row of any kind rather than at the end of its phase, so nothing measures a span and there is none to lerp across.
