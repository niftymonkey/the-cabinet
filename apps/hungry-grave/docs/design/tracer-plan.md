...
This is the dispatch-contract planning half from `docs/agents/feature-flow.md`: the definition, the verification steps, the module boundaries, the seams, and the test list. The coding dispatches execute it.

Ruling that opens this plan (Mark, 2026-08-19): the tracer starts the real base game. Built fresh from the blank scaffold per Hungry Grave ADR 0010, nothing lifted from `src/prototypes/ugly-slice`. `main.ts` is brought to feature flow Rule 1 in the same work.
...
- The bell tolls on its own clock from level 1, heard and seen and felt by the mobs it reaches, its damage falling off with distance so the far edge of a big ring tickles rather than kills (Hungry Grave ADR 0036).
...
7. On-device input check from the deployed URL after the input dispatch, steering the grave around an otherwise empty field. The feature flow makes this escalation mandatory for input-feel changes, and waiting until the end buries an input problem under five dispatches of content. Actor: Mark.
...
- `touch.ts`: uncapped drag steering onto the drag target. Steering is all it does: Mark ruled on 2026-08-22 that the belch binds to a dedicated corner button (ADR 0038), and the second-finger rule was deleted by dispatch 5 rather than left standing here.
...
- `main.ts`: rewritten to feature flow Rule 1. The router serves the game at the default route and the prototype list behind `#/prototypes`. Rule 1's specimen is the shape to copy, never the routing to copy: its routing goes stale the moment this rewrite lands.
...
Seven dispatches, each carrying the full contract from the feature flow. Dispatch 3 was split in two on 2026-08-20 and the sequence is eight items long as it stands.
...
4. The field: the three mob types, templates, stage rows, overlap resolution, corpses and freshness, plus a stubbed victory the moment the stage advances into its `over` phase. The stub matters more than it sounds: without it the only ending reachable before dispatch 6 is sealed shut, so the deploy at dispatch 5 would be a play of a game that cannot be won, and "goes start to finish" would first exist two dispatches from the end. With it, every deploy from here on is a complete run and that property never regresses. Ends with the deploy and a play by Mark. Plan: `docs/design/dispatch-4-field.md`.

   **Done.** What shipped stubbed, and what it means for anyone reading a dispatch-4 build: victory is a stub that fires on reaching the `over` phase rather than on the Undertaker's death (ADR 0007); both boss phases are empty and end on the tick they begin; the feast's never-decaying flag is tested and unused by any spawner; the mob silhouettes are placeholders and #38 owns the art; and nothing kills a mob except `clearingPolicy`, the test rig's stand-in for the storm, which dispatch 5 deletes. The Wall ships untested against ADR 0042's own property, because both bot policies that carry it need the belch. `dodgePolicy` does not survive the ramp on every seed and does not reach the `over` phase from the size ceiling; that is a finding about content authored against a build with no weapons in it, and the dispatch-4 report carries the numbers.
5. Weapon lines and the economy: the four lines, drops with per-line legibility, pricing, the belch, `sound.ts`. Ends with the deploy and play at verification step 9. Plan: `docs/design/dispatch-5-weapons.md`.

   **Done.** What shipped stubbed, and what it means for anyone reading a dispatch-5 build: the belch deals no boss damage, because there is no boss to take it (ADR 0008's boss half is dispatch 6's); the bell's rule that a boss takes its damage and never its push is unwritten for the same reason; both boss phases are still empty and end on the tick they begin; and victory is still dispatch 4's stub firing on the `over` phase. The five sounds are synthesized placeholders on the same footing as the placeholder art, generated by `scripts/make-sounds.mjs`, and no dispatch owns replacing them.

   Two findings the build produced rather than inherited. The drain-out was re-derived from twenty seconds to sixteen, not to the ten the plan proposed: measured across the five full-run seeds the field empties 15.0 to 15.5 seconds after a phase's last row, so the storm buys four seconds off the weaponless figure and no more. And ADR 0013's ten-to-twelve drops is not reached: a full run on real weapons produces 23 to 42 kills and 3 to 5 drops against a price table fitted to 268 authored mobs. The price table is not moved and `dodgePolicy` is not improved, both of which the dispatch plan forbids by name; the band is carried as a declared expected failure in `src/dev/bot.test.ts` with the cause written beside it, and the tuning dispatch owns the answer.
