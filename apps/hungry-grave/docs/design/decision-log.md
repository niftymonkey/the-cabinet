...
5. Grasping hands cut because their jobs are spoken for (close defense is the headstones', crowd punctuation the bells') and they read on the ground layer where corpse-versus-drop legibility lives. Close-defense orbiters are a proven archetype (King Bible and Garlic in Vampire Survivors). **Superseded 2026-08-27 by Mark's first-weapon-pool ruling (#76), which reinstates grasping hands as Territory's expression.** What it replaced: the first two reasons, both of which dissolved rather than being overruled. Close defence was the headstones' job and the headstones are gone from the pool; crowd punctuation was the bell's job and Repel keeps exactly the field-scale behaviour ADR 0036 rules for it, so it never took Territory's space. What stood: the third reason survives unchanged and is carried forward rather than waved away, because grasping hands do read on the ground layer where corpse-versus-drop legibility lives, and #31 has not yet read whether #36's drop-legibility fix worked. What the earlier ruling could not have known: that headstones would reach level 5 on a measured run and contribute almost nothing, and that the pool read by where and when each line acts would put two of its four lines at "at me", which is the overlap that made the swap the answer rather than a fifth line.
