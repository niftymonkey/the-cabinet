...
| ~~The quantisation grid is unset~~ **Closed by Mark on 2026-08-23: there is no grid.** | The dialogue said "0.01 base-speed units" and the agent's working figure was `int16` at 1/256, neither of which was put to Mark. Steering is stored as two `float32` and never quantised, and the measurement that retired the working figure is in the Question 5 paragraph above. |
...
- ~~**The quantisation grid**~~ **Closed by Mark on 2026-08-23: no grid, two `float32` a tick**, per the table above.
