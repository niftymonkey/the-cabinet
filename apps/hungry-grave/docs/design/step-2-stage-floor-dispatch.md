...
Every claim about existing code below cites a file and a line, read in this worktree while **path step 1 was mid-build**. That is the load-bearing caveat of this document and it is stated once, here, in full.

At the moment of reading, step 1 had landed its slice 1 (the skull stream rename): `src/game/lines/skullStream.ts` exists, `run.ts:11-12` imports it, and `soulStream` survives only at `src/game/__tests__/witness.test.ts`. Slices 2 to 10 of step 1 had not landed.

**Claims that will need re-checking against the tree once step 1 lands, by name:**

1. **Every line number in `src/game/run.ts`, `src/game/step.ts`, `src/game/events.ts`, `src/game/mobs.ts`, `src/game/corpses.ts`, `src/game/witness.ts`, `src/game/invariants.ts`, `src/game/faults.ts` and `src/tape/wireCodes.ts`.** Step 1 changes all nine (its plan, section 5). Locate every site by content, and a site you cannot find is a finding rather than something to skip (`docs/agents/lessons.md`, "a plan's line numbers go stale the moment its first phase lands").
2. **`RunState`'s shape.** This plan reads `run.ts:59-102` as it stands. Step 1 adds `roster`, `offer` and `bankedOffers` and deletes `killsSinceDrop` and `dropsPaid` (step 1 plan, section 4, `src/game/run.ts`).
3. **`spawnMob`'s signature.** This plan reads `mobs.ts:237-241` as `(state, type, order)`. Step 1 makes it `(state, type, order, carries)` (step 1 plan, section 4, `src/game/mobs.ts`).
4. **`cullMobs`'s return type.** `mobs.ts:408` returns `void` today; step 1 makes it return `SimEvent[]` and `step.ts:175` is its only caller (step 1 plan, section 4).
5. **`StageRow`'s shape.** This plan reads `stage.ts:13-23`. Step 1 adds the carrier column and puts a first schedule on `RAMP_ROWS` and `BACK_HALF_ROWS` (step 1 plan, section 5, the `src/game/stage/stage.ts` row: "This dispatch for the column and a first schedule; step 2 owns the authored schedule").
6. **`WITNESS_VERSION`.** `witness.ts:38` reads 4 today; step 1 moves it to 5 in its slice 2 (step 1 plan, section 10). This step takes it to one past whatever step 1 left.
7. **`FAULT_IDENTITIES` and `FAULT_IDENTITY_CODES`.** Step 1 appends three (step 1 plan, section 5). This step appends after them, and the wire codes are append-only (`wireCodes.ts:30-37`).
8. **`src/game/drops.ts` is gone** and `src/game/__tests__/drops.test.ts` with it (step 1 plan, section 5). Anything in this plan that mentions the drop path means the offer path.
...
Everything else cited below was read directly and is in files step 1 does not touch (`stage/templates.ts`, `caps.ts`, `tuning.ts`, `field.ts`, `clock.ts`, `grave.ts`, `mobFire.ts`, `palette.ts`, `layering.ts`, `sound.ts`, `audio.ts`, `engine.ts`). `stage/stage.ts` is **not** on that list and was on an earlier draft of it, which contradicted caveat 5 above: step 1 owns the carrier column on `StageRow` and a first schedule on both row tables, and both live in that file. Every `stage.ts` line number below is therefore a caveat-1 re-check like the other nine.
...
| --- | --- | --- | --- |
| Three named sections | 0049, 0050 | **Absent.** Two trash phases: `ramp` (`stage.ts:81-105`, 21 rows) and `backHalf` (`stage.ts:113-133`, 19 rows), chained through `PHASES` at `stage.ts:151-157` | Seven phases, three section tables, the one-property-per-section authoring, the names in `PhaseName` (`stage.ts:11`) |
...
| A section's property as a gate on the director's spend | 0047, 0050 | **Absent.** No live-count ceiling exists anywhere; `MOB_CAP` (`caps.ts:22`) is a bug detector and not a section's property | `Phase.liveTemplateCeiling` and `Phase.liveBodyCeiling` as rows step 4 reads before it spends, and the three property tests run under a stated killing policy rather than as always-on invariants |
| Phases chain on boundary events | 0006 | Built: `enterNextPhase` at `stage.ts:207-217`, `advanceStage` at `stage.ts:224-233` | Nothing structural; the loop at `stage.ts:225` exists only because stubbed boss phases end on the tick they begin |
| A phase's length | 0051, 0050 | **Superseded.** `phaseLengthTicks` at `stage.ts:173-177` is `last row t + DRAIN_OUT_SECONDS`, with `DRAIN_OUT_SECONDS` 17 at `stage.ts:62` | The end condition as a column on the phase: rows spent and the field clear at the two boss boundaries, the eye opening for the Crowd, the boss's death for the boss phases |
| The sparse last row | 0051 | **Absent.** The drain-out is silence in the rows and no row falls inside it (`stage.ts:24-27`) | The sparse rows, and the retirement of the silence |
| The Banshee | 0007 | **Absent.** `{ name: 'banshee', rows: [] }` at `stage.ts:153`, a stub whose reason is written at `stage.ts:145-150` | The whole boss: chunks, patterns, the feast, the ring emitter |
| The Wall | 0042 | **Half built.** The template is at `templates.ts:179-189`; its one row is `{ t: 2, template: 'wall', count: 22, type: 'shambler' }` at `stage.ts:114`, anchored per the comment at `stage.ts:107-112`. Never tested against its own property (`tracer-plan.md:176`) | The real anchor on the Banshee's death, and the two bot policies that carry the property |
| The feast | 0004, 0007 | **Mechanism built, unused.** `spawnFeast` at `corpses.ts:216-237`, `decays: false` at `:233`; its comment at `corpses.ts:211-215` says nothing spawns one yet. `FEAST_PAYOUT` at `tuning.ts:102` and `RESERVOIR_CAPACITY` at `tuning.ts:109` are the same number by construction | The callers: the Banshee's death, and both bosses' chunk breaks |
| The Undertaker | 0007, 0052 | **Absent.** `{ name: 'undertaker', rows: [] }` at `stage.ts:155` | The whole boss: three chunks, curtains, spiral, diggers, the gap rule |
...
| Victory | 0007 | **Stubbed.** `state.ending = 'victory'` at `stage.ts:215`, fired on entering the `over` phase, with the reason at `stage.ts:201-206`. `EndScreen.ts:15-17` says the same | Victory on the Undertaker's death |
| Sealed shut | 0003 | Built: `sealShut` at `grave.ts:161-163` through `runFloorLadder` at `grave.ts:171` | Nothing |
| The corpse cap never evicts | 0056 | **Contradicted.** `claimSlot` at `corpses.ts:160-177` evicts the oldest decaying corpse through `oldestEvictable` at `corpses.ts:144-151` and emits `corpseEvicted` at `corpses.ts:168-173`. The rationale is written at `corpses.ts:153-159` | The refusal, the fault, and the retirement of the eviction path |
| The corpse cap sized from scroll physics | 0056 | **Absent.** `CORPSE_CAP = 200` at `caps.ts:24`, a flat number; the surrounding comment at `caps.ts:10-20` derives `MOB_CAP` from a measured 51 mobs alive but derives nothing for corpses | The derivation, and the query over the stage rows it needs |
| Directed off-limits cells as data | 0047, 0056 | **Absent.** `StageRow` (`stage.ts:13-23`) and `Phase` (`stage.ts:135-138`) carry no permission column | Both columns |
...
| Stand-in music | 0049 | **Engine built, no caller.** `BGM` loops at `audio.ts:38`, cross-fades at `audio.ts:24-31`, and no-ops on an unchanged alias at `audio.ts:21`. `bgm.play` is called only in the prototype (`prototypes/ugly-slice/screens/TitleScreen.ts:128`). `sound.ts:61` returns null for `phaseChanged`, and `events.ts:241` says the music cue hangs there | The caller, the per-phase row, the bundle, the loop-gap trim |
...
| A section timeline off a tape | #39's instrument list | **Absent.** `src/dev/readings/readings.ts:50-66` declares twelve readings and none reads `phaseChanged` | A `sectionTimeline` reading |
...
- **Adding a layer name would be an ADR 0014 change.** `layering.ts:16-29`'s order is pinned as a literal at `src/app/__tests__/layering.test.ts:73-86` under the name `ADR_0014_STACK`, and `:141` asserts the container count equals `LAYER_ORDER.length`. The background therefore orders itself inside `ground` and adds no name.
...
6. **Golden digest, per behaviour-changing slice.** `GOLDEN` (`src/dev/digest.ts:276`) regenerates in the same commit as the behaviour, and the agent reads the diff field by field. The scenario runs 600 ticks from `SEED` 20260820 (`digest.ts:18-19`), which is inside what is now the Procession, so slice 2 moves it and later slices should not unless they claim to. A field moving that the slice did not claim is a stop-and-report, not a re-blessing.
7. **Headless run to victory, under the dodge bot at maxed levels.** From `apps/hungry-grave/`, a full-run test using `createRun`'s third parameter with `uniformLevels(5)` (`run.ts:154`, `run.ts:190`). This is named separately because the shipped bot only dodges and never levels a line (`docs/lessons.md`, "the bot is not a player"), so it cannot kill a boss from the birthright and the pinned loadout is the only way a headless run reaches the Undertaker at all. Passing means: `victory` fires, it fires on `bossKilled` for the Undertaker and not on a phase index, and no invariant fault fired.
...
`phaseLengthTicks` (`stage.ts:173`) and `DRAIN_OUT_SECONDS` (`stage.ts:62`) are deleted. Their readers are in section 7. `phaseSpent`, the rows-spent-and-field-clear predicate, survives as the module-private helper `phaseEnded` calls for the `rowsSpentAndFieldClear` case; it is not exported, because the phase's own column is what a caller should read.
...
- **`GraveHitSource` (`grave.ts:28`)** is `MobType | 'contact'` and is what `hitGrave` is handed at `step.ts:53`, straight off `shot.emitter`. It widens with the emitter.
...
`oldestEvictable` (`corpses.ts:144`) is deleted and `claimSlot` loses its `events` parameter, because it no longer has anything to report.
...
// MobFired (events.ts:125-130) gains the fire kind beside the emitter it carries.
interface MobFired {
...
| `src/game/step.ts` | Changed | The boss tick and the set-piece tick take their places in the stated order. The storm's pass reaching them is `stormTargets.ts`'s row above and not this one. The belch-before-every-overlap rule at `step.ts:150-153` is guarded through the change, and `step.ts:53` hands `hitGrave` a widened emitter. | This dispatch |
| `src/game/events.ts` | Changed | Six new payloads; `CorpseEvicted` retires. | This dispatch |
| `src/game/witness.ts` | Changed | `WITNESS_VERSION` moves once; folds the boss and the set piece; `foldStage` (`witness.ts:267-270`) gains nothing, because the phase state's shape is unchanged. | This dispatch |
...
| `src/game/faults.ts` | Changed | Appends the new identities to `FAULT_IDENTITIES` (`faults.ts:18-31`) and their severities, after step 1's. | This dispatch |
...
| `src/dev/readings/readings.ts` | Changed | Registers the new reading in the three places the graph is declared (`readings.ts:50-66`, `:76-95`, `:97-110`). | This dispatch |
...
4. *Under a hand that kills what arrives, the Procession never has more than one template live.* Pins `game-concept.md:48`: "the first owns emptiness, never more than one template live." The killing policy is part of the assertion and is stated in the test: the sharp hand, or a pinned full build through `createRun`'s third parameter with `uniformLevels` (`run.ts:170`). The section's ceiling is `Phase.liveTemplateCeiling`, which the director at step 4 may not spend past (ADR 0047), and the test reads it from the phase rather than restating it. The deliberate absence beside it, in the same file: a slower hand that leaves two templates on the field raises no fault, because the authored rows are the floor and ADR 0023 puts every invariant in the player's own build.
...
45. *Victory fires on the Undertaker's death and not on reaching a phase.* Pins ADR 0007's ending through `game-concept.md:70`: "His death is the ending." Retires the stub at `stage.ts:214-216`, whose own comment at `stage.ts:201-206` says it is one.
...
48. *Both endings are reached across the seeds, so neither is unreachable.* Pins ADR 0028's `ending` field being sealed, victory or absent, and carries forward the existing assertion at `bot.test.ts:358-366`.
...
52. *Nothing already on the field is ever removed.* Pins `caps.ts:64-67`'s own rule, guarded through the change: "a shot the player has read and started dodging cannot vanish."
...
98. The tick runs the belch before every overlap pass, guarded through the boss and set-piece additions (`step.ts:150-153`'s stated rule).
...
113. `ADR_0014_STACK` (`:73-86`) and `LAYER_ORDER`'s length (`:141`) are unchanged, which is the proof the background added no layer.
...
**`DRAIN_OUT_SECONDS` (`stage.ts:62`): deleted.**
Readers: `stage.ts:176`; `stage.ts:239`; `src/game/stage/__tests__/stage.test.ts:23,92,94,98,107`.

**`phaseLengthTicks` (`stage.ts:173`): deleted, replaced by `phaseEnded` (with `phaseSpent` surviving as its module-private helper).**
Readers: `stage.ts:229,237`; `src/game/stage/__tests__/stage.test.ts:25,93,96,97,101,106,170,186,218`; `src/dev/__tests__/bot.test.ts:27,124,125`.

**`RAMP_ROWS` (`stage.ts:81`): renamed `PROCESSION_ROWS` and re-authored, and it moves to `rows.ts`.**
Readers: `stage.ts:152,240`; `src/game/stage/__tests__/stage.test.ts:26,94,113,125,154`; `src/dev/__tests__/bot.test.ts:28,128,420`; and **fifteen test files that import it only to silence spawns with `run.stage.firedRows = RAMP_ROWS.length`**: `src/game/__tests__/corpses.test.ts:29,46`; `src/game/__tests__/mobs.test.ts:41,66`; `src/game/__tests__/belch.test.ts:13,18`; `src/game/lines/__tests__/bell.test.ts:15,30`; `src/game/__tests__/storm.test.ts:21,31`; `src/game/__tests__/mobFire.test.ts:38,57`; `src/game/__tests__/caps.test.ts:16,20`; `src/game/__tests__/step.test.ts:18,194`; `src/game/__tests__/drops.test.ts:21,25` (this file is deleted by step 1); `src/game/lines/__tests__/skullStream.test.ts:18,34`; `src/game/lines/__tests__/wisps.test.ts:14,29`; `src/app/screens/game/__tests__/StormRenderer.test.ts:13,26`.
...
**`BACK_HALF_ROWS` (`stage.ts:113`): replaced by `CROWD_ROWS` and `VIGIL_ROWS`, and it moves to `rows.ts`.**
Readers: `stage.ts:70` (a comment), `stage.ts:154,241`; `src/game/stage/__tests__/stage.test.ts:22,98,125,136,154`; `src/dev/__tests__/bot.test.ts:25,128,134`.

**`PHASES` (`stage.ts:151`): five entries become seven, and each gains `ends`, `directed`, `music`, `liveTemplateCeiling` and `liveBodyCeiling`.**
Readers: `stage.ts:213,225,226,229,242`; `src/game/stage/__tests__/stage.test.ts` (throughout, via its `phase()` helper); `src/dev/__tests__/bot.test.ts:124,125,140-143`.

**`PhaseName` (`stage.ts:11`): `'ramp' | 'banshee' | 'backHalf' | 'undertaker' | 'over'` becomes `'procession' | 'banshee' | 'crowd' | 'waking' | 'vigil' | 'undertaker' | 'over'`.**
Readers in production: `stage.ts:136,244`; `src/game/events.ts:7,244`. In tests: `src/game/stage/__tests__/stage.test.ts`; `src/dev/__tests__/bot.test.ts:140-143`. The prototype's own `PhaseName` (`src/prototypes/ugly-slice/game/types.ts:88`) is a name collision and not a reader (ADR 0010).

**`CORPSE_CAP` (`caps.ts:24`): 200 becomes a derived value, initially 232.**
Readers: `caps.ts:123`; `src/game/corpses.ts:5,108`; `src/game/invariants.ts:8,401`; **`src/app/screens/game/FieldRenderer.ts:3,154,155`**; `src/game/__tests__/corpses.test.ts:9,341,352,359`; `src/game/__tests__/run.test.ts:9,37`; `src/game/__tests__/caps.test.ts:9,102,106,122,128`; `src/app/screens/game/__tests__/FieldRenderer.test.ts:9,89,100,333`.
...
**`oldestEvictable` (`corpses.ts:144`): deleted.**
Readers: `corpses.ts:166`.

**`corpseEvicted` (`events.ts:146`): deleted with the path that produced it.**
Readers: `corpses.ts:169`; `events.ts:147,278`; `src/game/__tests__/caps.test.ts:111,142`; `src/app/__tests__/sound.test.ts:37`. It carries no wire code, so no format version moves.

**`WITNESS_VERSION` (`witness.ts:38`, 4 today, 5 after step 1): moves once, in the first behaviour-changing slice of this step.**
Readers: `witness.ts:318`; `src/tape/playback.ts:8,121,167,265`; `src/app/tapeHeader.ts:7,82`; `scripts/record-conditioned.ts:24,147`; and the test sites across `src/tape/__tests__/`, `src/dev/__tests__/`, `src/app/__tests__/`, `src/app/screens/__tests__/` and `scripts/__tests__/`, all of which read it symbolically rather than pinning a number, so none needs editing. Step 1's plan established that; re-verify it holds after step 1 lands.

**`FAULT_IDENTITIES` (`faults.ts:18-31`): three appended, after step 1's three.**
Readers: `faults.ts:34`, the severity table below it, `src/game/invariants.ts`, `src/tape/wireCodes.ts`'s `FAULT_IDENTITY_CODES`, and the codec tests. Append-only by the wire rule (`wireCodes.ts:30-37`).

**`Shot.emitter` (`mobFire.ts:70`) and `MobFired.emitter` (`events.ts:127`): `MobType` becomes `MobType | BossKind`, and both gain a `kind` beside it.**
Readers of the emitter: `mobFire.ts:82,141,147`; `step.ts:53` (which hands it to `hitGrave`); `grave.ts:28`'s `GraveHitSource`; `src/game/__tests__/mobFire.test.ts`; `src/app/__tests__/sound.test.ts`. Readers of the new kind: `mobFireSprite.ts:57` and `:99`, and nothing else, because the kind exists for the draw.
...
**The loop gap is measured first, and nothing is built for it until the measurement says so.** `handoff.md:33` names mp3 loop gaps and the LAME headers the encoder wrote, and an earlier draft of this plan turned that into an in-app header parse: decode the file, read the encoder's delay and padding, slice the buffer by them. That premise is stale. Firefox has trimmed on decode since 83 (Bugzilla 1566389), Chrome and WebKit both do the same (WebKit 223519), so the buffer the app gets back is already trimmed and the parse would be a decoder shipped inside a game to work around a browser bug that no longer exists. Verification step 13 runs before the music slice and reports the three loops' real wrap. If a gap survives anyway, the answer is `start` and `end` in seconds as data rows beside that loop's alias, which `@pixi/sound` 6 already maps to the source node's `loopStart` and `loopEnd` (`WebAudioInstance.mjs:168-174`), and which is three numbers rather than a parser. Module test 129 guards the absence either way.

**The background is one tiled ground plus a per-section dressing set, drawn into the existing `ground` layer.** The layer exists and has no consumer (`layering.ts:17`), it is already masked to the field (`GameScreen.ts:171-173`), and using it rather than adding a name keeps ADR 0014's stack literal (`layering.test.ts:73-86`) untouched. The sets are named in the design record's section 7, from `assets-staging/crawling-depths/`.
...
**Slice 4. The carrier schedule on the new rows.** Eight, eleven and six, on File and V rows in the Procession and never on a Drip. Spec tests 53, 54 and module tests 120 and 69's re-check land here.

**Slice 5. The corpse cap.** Derived from all four peak-arrivals terms, never evicting, the refusal, the fault, the sprite pool following it. The pour's own rows and `POUR_SHARES` land in `rows.ts` here rather than with the set piece at slice 10, because this is the slice where `peakArrivals` first needs them and where putting them in `setPiece.ts` would close the import cycle section 4 describes. **The corpse-refusal fault identity and its wire code land here too, with the refusal that raises them**, not at slice 6 with the boss identities: a fault the code can raise while its identity does not exist yet is a fault that reports as unknown, and an earlier draft split them across the two slices. `WITNESS_VERSION` does **not** move here. Spec tests 49 to 52 and module tests 87 to 89, 97, 109, 121, 128 land here.
...
**Slice 9. Both endings, as code.** Victory on his death, the stub at `stage.ts:214-216` retired, the hit-taking policy. Spec tests 45 to 47 land here, each driven from a pinned phase rather than from a run. The endings' headless runs are slice 10's and not this slice's, because the Crowd ends on the eye opening (spec test 115) and no run reaches the Vigil before the source exists.
