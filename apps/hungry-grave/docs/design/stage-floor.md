...
**Amended 2026-09-08, Mark's ruling on #104:** "it is shootable, so killing it ends the moment early" becomes: it is shootable, and killing it removes the source's body while the pour finishes anyway. The remaining budget keeps pouring from the pour point on the same clock until it is spent. What stood: the shootability, the immunity while dormant, the pour and its one point, the drag across, the silence, and the cooldown row. What changed: what the kill costs, which is now the body and the rest of the source's stay and never the rest of the trail, so the hand that commits up the trail and fights the source down never ends its own food early. What the paragraph could not have known: it was written before the property was measured, and the forfeit it implied paid the hand that held back over the hand that committed, which is the opposite of what the pour is for.
...
**How long the body can be shot at is a data row, and the pour runs independently of it (#104).** Against the sharp hand the body outlasts the pour; against any hand, a kill takes the body and the remaining budget keeps pouring. Its health is that row. The derivation, all initial. A full build at full uptime on one body directly above the grave does about 150 points of storm a second: the skull stream's five columns at eight damage a skull every eighteen ticks (`skullStream.ts:25`, `skullStream.ts:86`, `skullStream.ts:41`), which is 5 x 8 / 0.3 = 133.3 a second, plus the bell's forty at close range every hundred and eighty ticks (`bell.ts:54`, `bell.ts:32`), which is 40 / 3 = 13.3 a second, for 146.6. Territory and the wisps are left out of that figure rather than counted in it: Territory claims the densest knot of mobs ahead of the grave and the wisps fire on a swallow and hunt bodies, so neither is a dependable share against one stationary source. Both add on top, and the harness measures the real number at step 4. The pour is 75 bodies at one every twelve ticks, which is fifteen seconds, so the floor on the source's health is 150 x 15 = 2250 and the initial row is **2400**, a second of margin above it.

**Amended 2026-09-08, Mark's ruling on #104:** the derivation above ("sized so the pour completes under a full build") has lost its reason, because the pour now completes whatever the storm did. The row stays as an initial data row for the step 4 harness, and its reason is now the source's stay: how long the body remains a target the storm can work on and a thing on the ground to read. What stood: the row, the arithmetic behind it, and the exclusion of Territory and the wisps from the estimate. What changed: only what the number is for. What the paragraph could not have known: it was written while the health row was the whole of the guarantee that the pour finishes, and the ruling now carries that on its own.
...
**The shape is Gradius's Moai: vulnerable only while open.** The dormant source takes no damage at all; the storm can touch it only once it has opened and is pouring. Gradius's ordinary Moai are background statuary that die to a shot in the mouth, and the mouth is there to shoot only while the statue is firing (`docs/research/set-piece-and-final-boss-chunk.md` section 2). A fast kill never deletes the moment: it ends the source's stay rather than its pour, which is Ikaruga's bunretsu read the same way round, where speedkilling the midboss buys the player a longer payout rather than a shorter one (the same record, section 2). What the storm buys is the body's departure and the sight of an empty pour point; what it never buys is a shorter trail.

**Amended 2026-09-08, Mark's ruling on #104.** The paragraph that stood here refined `game-concept.md:50`'s "killing it ends the moment early" into killing it ending the source's *stay* early, and carried that as a commitment for Mark to overrule. He has ruled, and the ruling is the same conclusion made unconditional: killing the source removes its body and nothing else, the remaining budget keeps pouring from the pour point on the same clock, and there are two close reasons rather than three, the budget spent and the source off the bottom edge. The kill is its own event beside them. What stood: the immunity while dormant, the health row, the budget, the rate, the pour point's drift and sweep, and the Waking phase ending the tick the pour is over. What changed: the kill no longer ends the pour, and it no longer starts the Vigil's cooldown early either, because the source stays on the run until its budget is spent. What the paragraph could not have known: it read the health row as what made the two the same thing in practice, so the guarantee it stated held only while a hand could not out-damage the row, and the ruling makes it hold whatever a hand does.
...
The Waking pours a finite budget and ends on a condition, never on a timer. That is the industry standard for a mid-stage set piece and the research found no shipped duration in seconds for any of them (research record, "What is solid" claim 1 and the Open items). It ends two ways: the budget spent, or the source off the bottom edge. Killing the mouth was a third until Mark's ruling on #104, and it is now an event that takes the body and leaves the pour running.
...
- **The ground itself**, tiled from `Terrain/Tiles.png`, scrolling at half the field's scroll so it reads as parallax depth under the mobs. One tile set for all three sections, because the rock is the same rock.
  ...
  **Amended 2026-09-08, Mark's ruling after the 13b deploy:** the ground scrolls at the field's own scroll and not at half of it, because Territory is lobbed onto the ground and belongs to it, and a patch drifts in the sim at `SCROLL_SPEED`; at half, the rock slid out from under every landed patch and the field stopped reading as one place. The parallax row is gone rather than set to one. **Amended in the same breath:** the ground is not tiled from `Terrain/Tiles.png` at all. That sheet is twenty bordered blocks lit at twenty angles, and tiled whole it read as a lattice of loose blocks placed at random and spun. The floor is baked at import from an authored layout of five of those cells, laid as large slabs whose grooves survive only where two slabs meet, so the floor reads as one paved piece of ground. The bake lives in `scripts/grayscale-import.ts`; the runtime still draws one tiling texture and knows nothing of the layout. **Amended 2026-09-21, Mark's ruling that the prototype's ground goes on the whole field:** the floor does not come from the pack at all. It is painted at run time from the grave prototype's own `paintGround`, ported with its colours, its wide patches, mottles, cracks, grain, grass tufts and gravel, and baked once a run into one texture the size of the field. What stood: one ground for all three sections, because the rock is the same rock; the ground at the field's own scroll; the one tiling texture and the mask on the `ground` layer; and the dressing, which is still imported art on a seeded stream and still desaturated on import. What changed: where the floor's picture comes from, and what a sprite has to read against, because the earth moves from `nightSpeckle` at luma 13.99 to `groundNight` at 30.54 and every sprite over it loses about twelve APCA points, all of it pinned in `palette.test.ts`; and the grass tufts are the ground's now, which is where the grave's own tufts went (`grave-in-the-ground.md` R8). What the paragraph could not have known: that the grave would be ported from that prototype line for line, and that the prototype's grave is cut into a ground brighter than its own walls, so on a near-black floor the ported grave's bites, trodden margin and crumbs vanished into the earth around them. The slab bake still runs and `src/manifest.json` still carries its `floor.png`; nothing under `src` reads it any more.
...
| Name | Role | Target |
| --- | --- | --- |
| `groundNight`, with `groundSpeckle`, `groundCold`, `groundWet`, `groundDamp`, `groundCrack` and `groundGravel` | the ground the field stands on, painted over the whole field, every section | the prototype's own earth: base `0x454f5d` luma 30.54, with the pale fleck, the damp and dry patches, the cracks and the gravel laid over it |
| `groundDressCold` | the Procession's statues and cliff | hue about 216, luma 38.56, re-valued with the ground on 2026-09-21 so it keeps the eight luma it stood above the old tile |
| `groundDressWet` | the Crowd's urns and veins | hue about 165, luma near 24 |
| `vigilTint` | the Vigil's eyes, tentacles and floating rocks; the one departure | hue about 190, luma near 30, the highest saturation of the ground's own colours |
...
`nightSpeckle` (0x1d2434, luma 13.99, `palette.ts:33`) was the ground tile's colour from this record until 2026-09-21, when the ruling above took the floor to the prototype's own earth. It is declared and drawn nowhere in `src/app` again, and it is still the darkest ready-made earth the palette holds.
