...
- **The silhouette mapping shipped known-wrong, and was swapped before #31 ran.** The soul stream held the tall bar and the headstones the disc, while the natural imagery is the reverse; #31 is the playtest that reads whether a drop says which line, and running it on a mapping already scheduled for inversion would have made any tester confusion unattributable. #76 then replaced the headstones with Territory, so the tall box now holds a grasping hand and the disc is the soul stream's.
