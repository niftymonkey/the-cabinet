...
## 10. Found by the implementation gate round, 2026-08-21

These are the implementation round's findings, recorded rather than acted on. Nothing here was folded into the code: the dispatch shipped as planned and these are what the next planner inherits. Every figure was re-measured in the main thread before it was written down.

### The plan contradicts itself, and section 8 is the correct half

**Section 5's ceiling test cannot pass and section 8 says why.** Section 5 asserts `dodgePolicy` reaches the `over` phase from the size ceiling. Section 8 computes that a ceiling start "buys a longer look at the back half and not a win", because the Wall costs seven hits on its own and leaves nine for 66 seconds carrying 145 unkillable mobs. Measured: the bot seals in the back half on all five seeds, at 140 to 168 seconds, after 18 hits. Section 8's arithmetic is right and the test is the wrong half of the same document. Do not fix this by improving the bot.

**`dodgePolicy` also does not survive the ramp on three of five seeds**, sealing at 83, 89 and 107 seconds of a 125-second ramp, and the plan predicts this nowhere. Attributed: **15 of 18 hits across the five seeds are aimed mob fire and two are body contact.** Every hit lands between t=65.8 and t=106.6, and 15 of them between t=81 and t=89. Concurrent airborne shots run 0 to 3 for the first 43 seconds, 3 to 8 through t=70, then peak at **19 at t=82 to 85**.

The cause is the ramp's `file 4 revenant` at t=74 landing on a field already carrying eight armed shamblers. Nothing kills a mob here, so each armed mob lives about fifteen seconds and fires five or six times instead of dying before its first volley. **The missing weapons do not inflate the ramp uniformly: they inflate it multiplicatively on the armed share**, because an armed mob's shot output is a function of how long it lives. Mob counts are probably fine. Mob fire is off by roughly a factor of five, in a direction the plan did not anticipate. Section 8 told Mark to expect to be sealed at the Wall around t=122 by contact; he will be sealed forty seconds earlier by aimed fire.

### Two content findings that survive weapons

- **The ramp's pressure peaks at t=74 and then declines into its own boundary.** After the revenant File there are no more revenants in the ramp at all: the last five rows, t=88 to t=105, are shamblers only at one-in-three armed. Armed count runs 8 to 10 during t=74 to 90 and 6 to 8 during t=90 to 105. That shape is authored and holds whatever weapons do to the magnitudes, because it comes from where the always-armed type sits. A teaching ramp should be hardest when it hands off to the Banshee; this one eases for seventeen seconds and then goes silent for twenty. Suggested edit: move or split `file 4 revenant` so the always-armed type reaches the boundary.
- **The ghoul has no develop rung.** Lone Drip at t=62, then a V of seven at t=83, and that is every ghoul in the ramp. The shambler gets 1, 1, 3, 5, 2, 3 before its first V; the revenant gets 1 then 4; the ghoul gets 1 then 7, on the type the record says is only answerable by positioning, and its lesson lands in the same ten-second window as the fire peak. Suggested edit: a Drip of 3 or a V of 3 between t=62 and t=83, ideally in the t=88 to 96 lull the finding above identifies.

Both are row-table edits and both make the ramp better under the storm too. Both are Mark's, and the trigger is the dispatch-5 play when the fire count is real.

### Readability, found by eye and by measurement

- **Mob sprites are materially smaller than their hitboxes.** `polygon(sides, radius)` puts vertices on a circle, so the ghoul's triangle at radius 9 has a bounding box 18 wide by 13.5 tall inside an 18 by 18 hitbox, leaving 4.5 units of dead air across its whole top edge. The revenant's diamond and mob fire's five-point star fill their boxes only at the edge midpoints, so both carry invisible margin in the diagonals. The convention runs the other way, and the plan gave the grave's rim the opposite treatment deliberately: its drawn outer edge is exactly its hitbox. It cuts hardest on the ghoul, whose whole threat is its body and whose stated counter is shaving its edge, and worse because the sprite rotates to its heading while the box stays axis-aligned. Growing the sprites is the cheaper direction and does not touch the Wall's width arithmetic. Trigger: Mark's read of whether he is being hit by nothing.
- **Mob fire renders about 7.2 CSS pixels on a 390-wide phone.** `shotHalfExtent` is 5, so a shot is 10 field units, and at 0.72 CSS pixels per unit that is the exact size section 4.15.5 rejected for the drop as "under half Android's 16dp small-icon floor". ADR 0014 calls mob fire large. The star's points mean the drawn extent is smaller still in most directions. This is the thing a player must see to dodge, and 15 of 18 bot deaths are mob fire.
- **The revenant's tell loses salience as the shot gets closer.** `tellRadius` closes from 0.9 to 0.2 of the half-width, so at the instant of firing it is a 2.2-unit ring, about 1.6 CSS pixels of radius on a phone. A closing iris reads as a countdown conceptually and falls to nothing perceptually at the moment of maximum urgency. The convention is a wind-up that builds into the release. Keep the closing read and pair it with something that grows.
- **The armed marker reuses the ghoul's silhouette.** `drawArmedMark` draws a down-pointing triangle, which is the ghoul's own body shape, at 0.55 of the mob's half-width: the ghoul is radius 9 and the mark is radius 6. ADR 0014 makes silhouette the first discriminator between types, and this spends that channel on a fourth meaning at a size where the two are three units apart. It does survive grayscale, which was the assertion asked for.
- **The corpse last-chance flicker runs at 5 Hz.** `FLICKER_HALF_PERIOD` is 6 ticks, so a full period is 0.2 seconds. `tuning.ts` already derives `INVULNERABLE_TICKS` from WCAG SC 2.3.1's three-flashes-per-second limit, computes a 21-tick floor and takes 24 for margin, and records that SC 2.3.1 has no essential-to-functionality carve-out. The same derivation gives a floor of 11 ticks per half period. A single corpse is small enough for SC 2.3.1's small-area exemption, which is why this is not a defect today; the burst-kill lockstep case section 9 defers to dispatch 5 is where the area adds up, and the rate is the number that decides whether that deferred case is a hazard.

### The outline construction costs six times what section 9 priced

- **`SEPARATION_EXCEPTIONS` shipped at 29 pairs where section 4.15.5 priced four**, and 18 of the 29 involve a sprite dispatch 5 draws: seven against `skull`, eight against `splash`, and one each for `stone`, `belchEruption` and `graveGlow`. The extra pairs are real discoveries the plan never measured, because it only priced `graveRim`'s own crossings. The bill arrives in the dispatch already carrying the most.
- **Assertion 1 is recorded rather than inherited.** As written in section 4.15.5 it would have failed on the day it was added, so the implementation shipped an `AWAITING_A_COMPANION` allowlist naming `graveGlow`, `skull`, `stone`, `wisp`, `bellRing`, `belchEruption` and `splash`, each with the reason "drawn by dispatch 5". The test passes today and keeps passing after dispatch 5 draws them, so nothing fails when the wisp renderer lands. **Dispatch 5 must delete each entry as it draws that sprite**, and a planner who never opens the allowlist never learns it exists. The implementation had no honest alternative; the plan was inconsistent between 4.15.5 and section 9.

### Vocabulary and ownership the record does not yet carry

- **The cancel scatter is shared vocabulary and lives only in section 4.14.** The plan committed it in as many words: use the same read the belch will use, "so cancellation has one vocabulary from the start". The code carries it as a `Scatter` type and a `drawScatter`, it is player-visible, and dispatch 5's planner has no reason to open a finished dispatch's plan. It belongs in `CONTEXT.md`.
- **The renderer's feel magnitudes are ADR 0014 announcement channels, not dressing.** `HIT_DIM_ALPHA` at 0.5, `CORPSE_FADE_FLOOR` at 0.25, the flicker's period and depth, the scatter's reach and spoke count, and the countdown's blur strength were all invented in this dispatch and are all player-visible. Section 9's magnitudes bullet names sim numbers only. #38 will read the hit dim's alpha and the corpse fade floor as dressing it may re-decide, and both are announcement channels.
- **The countdown blurs `mobBodies` and `mobFire` and spares `corpses`.** That matches the plan's literal words and is invisible in this build, because a played run produces no corpses. From dispatch 5 it is the exact line the blur was written against: corpses are what a dive is planned through and freshness is a deadline, so three sharp seconds of corpse layer buys the highest-value read in the game. The plan's own rule, never occlude the channel the player is being asked to re-read, covers the grave and the rim and does not cover corpses. Trigger: dispatch 5.

### What the bot cannot tell anyone

**`dodgePolicy` is structurally incapable of the ghoul's own counter.** It picks the move maximizing the tightest clearance over samples at 5, 12, 20 and 30 ticks, capped at 60 units, and projects every threat at constant velocity. Cutting hard across a chaser means accepting less clearance at 5 and 12 ticks to get behind its turn, which that scoring dominates, so the policy always flees radially; and the ghoul re-aims thirty degrees inside the policy's own half-second lookahead. Its one ghoul contact death was the lone teaching ghoul at t=68.2, which is a fact about the bot and not the content. **Ghoul fairness is unread rather than judged**, and ADR 0016's turn-rate bound has no evidence behind it yet. Do not improve the bot; read it by hand.

### Found and fixed in the same round

Two things were fixed rather than recorded, because both were shipped-quality defects in the class this app keeps repeating and both were in the path of the first play.

- **The fifth pooled-screen leak, in `FieldRenderer`.** `shotMemory` and every scatter's `born` survived a reset, and neither is a draw cache: both compare against a value that resets with the run. A run ending with shots still on the field made `syncShots` read alive-then-dead across the boundary, so run two opened with a cancel scatter for every one of them, at run one's positions, for a fifth of a second on a screen the player had just chosen to replay. The quieter half lasted longer: `oldestScatter()` takes the smallest `born`, and a `born` carrying run one's tick is larger than anything a fresh run produces for its first fifteen seconds, so those slots were never reused and the pool ran at half size while the cancel read degraded. Fixed with a `forgetPreviousRun()` called from `attach()`, which is the one place a renderer is put back, because `reset()` calls `layers.clear()` and then `dressField()`.
- **The test written to catch that leak could not see it.** `screenLifecycle.test.ts`'s "starts with an empty field" played run one for 200 ticks. The ramp's first row is a Drip of one, index 0 is never armed under the modulo-three rule, and the first armed mob arrives at t=14, so run one had no mob fire at all and the assertion over the `mobFire` layer was vacuous. It now puts a live shot on the field before the reset. Verified by disabling the fix and watching the test go red, because a guard nobody has seen fail is a claim. **The cancel scatter had no test of any kind**; it now has three, covering the read itself, the culled-shot case that must not scatter, and both halves of the leak.

### Recorded, for dispatch 5 and later

- **The digest's fold is coarser than the divergence it exists to catch.** `fold` does `Math.round(value * 1e6)`, and one f32 ulp at the ghoul's turn cosine is about 1.19e-7, below that quantum. Measured: the fold moves at a delta of 1e-6 and does not at 1e-7. A single-tick divergence of exactly the size ADR 0015 exists to catch is invisible and is only caught once it accumulates into position, which it does over the scenario's 600 ticks, so the digest is not broken. But the test asserts detection at 1e-5, a hundred times coarser than the target, so nothing pins the instrument's real resolution and a later shortening of the scenario would silently cross the line. Fix is a finer fold or a perturbation at ulp scale in the assertion.
- **The digest still makes zero RNG draws.** Measured after the extended scenario: `spawns.drawn` 0 and `mobFire.drawn` 0. The only rows inside the 600-tick window are two Drips of one; a Drip draws nothing and index 0 is never armed. The first stream-drawing row is the File at ramp t=20, which is tick 1200. `drawn` is in the digest so that a divergence in how many draws a tick made is visible, and the scenario makes none, so section 4.13's claim that the extension "buys coverage of the spawn sequence" buys spawn positions for two Drips and leaves the RNG cursors as unexercised as before. Cheap fix: run to 1300 ticks, or script a Rain the way the ghoul is scripted.
- **A File above count 6 stops being a file.** `rankStep` closes a group up against `MAX_ENTRY_DEPTH` rather than reaching past it, which is a stronger guarantee than the plan asked for and is honestly commented. The consequence is that the File's stated identity, one body length behind the last, holds only to count 6: a File of 7 gets 22.3 units of spacing against a 26-unit body and a File of 12 gets 12.2, so the bodies overlap and the lane stops reading as a lane. Today's rows top out at 6 so nothing is wrong now. It matters at dispatch 7, where raising a File count is meant to be a pure density edit, and "count lives on the row so density tuning never edits a playtest-proven shape" is exactly the promise it breaks. Trigger: the first File count above 6.
- **A `BlurFilter` is allocated per countdown and never destroyed.** The pattern matches the pre-existing `PausePopup` and `SettingsPopup`, so it is not a regression, but the trigger frequency changed: the countdown fires on every resume and on every return from a backgrounded tab. Pixi's own guidance is to share one filter instance and toggle `filter.enabled` rather than rebuilding the filters array, which would also settle the unhandled rejection this raises in `screenLifecycle.test.ts` under node, where `new BlurFilter` has no document to compile a shader against. The filters still apply and the assertions pass, so it is noise rather than a failure.
- **The reserve assertion is arithmetic, not the pixi measurement section 4.16 specified.** The plan put it in `GameScreen`'s test "where pixi is available"; pixi cannot measure text without a document, so the landed test computes `WIDEST_STACK_LINE.length * METER_FONT_SIZE * 0.62`. The bound is defensible and the comment is candid, but the plan's stated reason for the assertion's location no longer holds and nothing measures the real width until the rendered check. Worth knowing when a longer readout is added.
- **`dodgePolicy` does not price the field edge.** `graveAfter` clamps the projected position to the field, so a move into the wall scores identically to standing still. The grave genuinely is cornered when it dies: one seed takes four hits in 2.3 seconds against the right edge and another dies against the left. A capped room-to-move term was written and re-run across all five seeds and it saved nothing, sealing a seed that had previously survived, so this is not what kills the bot. Record it for whoever rewrites the policy in dispatch 5.
- **An armed mob that has passed the grave keeps firing upward at it from behind.** Mobs are culled only past the bottom edge, and shots were observed leaving from y=734 and y=763 at a grave at y=711. It follows from the plan's own aiming rule rather than being a bug, and under the storm it is close to zero shots, but here it is a real fraction of them and it will read as unfair on device. Another thing the weaponless build exaggerates.
- **The ghoul's turn rate, 60 degrees per second, is a number this plan did not give.** It yields a 90-unit minimum turn radius in a 540-wide field, which sits inside ADR 0016's "cutting hard across it beats it", and the pair test holds it without asserting a magnitude. Dispatch 7 owns it.
