...
**SUPERSEDED 2026-08-22, after play. Only the first drop of a run seeds; every later drop rolls uniform over all four lines. ADR 0002 carries the current rule and the reason. The paragraph below is kept as the reasoning that was true when this dispatch was built.**
...
- **`docs/adr/0002-corpses-are-fuel-and-carriers-meter-power.md`: the drop dice.** The ADR currently says the dice only pick which line levels, and section 6.8 now also rules how they pick: seed the first drop of each unowned line, then roll uniform. That is a rule about the shape of a run, Mark decided it on 2026-08-22, and its durable home is the ADR rather than a dispatch plan nobody reads again.
...
- ~~**No run of four or more drops can miss a line**, which is the whole point of seeding and is the measure the weighted scheme could only push to 1.25%.~~ **Superseded 2026-08-22**: with only the first drop seeded a run can miss a line, at about 5.6%, and the test now asserts that it can. See ADR 0002.
