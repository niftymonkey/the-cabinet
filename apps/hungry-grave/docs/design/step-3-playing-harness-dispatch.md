...
**Two things this plan found that the record does not carry, both named again in section 8.** The airborne-projectile figure #39 asks the report for is not readable off a headless tape today: shots on the field are sampled only at expensive-frame ticks (`replayTallies.ts:158`, `densityOf` at `:74-80`), and a headless batch tape carries no frame rows at all, so the sample set is empty. And the record's batch-cost estimate is soft by about a factor of three: it derives two seconds per stage length from `bot.test.ts:700`'s thirty-second budget for five runs and then bills each whole run at one stage length, where `MAXED_RUN_TICKS` (`bot.test.ts:355`) is three stage lengths.
...
**The hand is a seventh policy beside the six, and it belches (ADR 0053, ADR 0042, the record's sections 1 and 12).** ADR 0053's list is four verbs and this makes it five. The reason is filed rather than assumed: #37's story 12, one belch plus play beats the Undertaker, has no hand that can answer it, and #39's batch report inherits the boss-fight belch as an instrument reading no current policy can produce, which is #98's third comment. Under a hand that never belches, `belchCadence` reports an empty fire list and a run's whole length as waste, which is an instrument reading its own absence. The cost, eyes open: the belch is a spend judgement and neither knob touches it, so the sloppy hand belches exactly as well as the sharp one.
...
10. **The first batch, sharp corner.** 48 seeds under `steady-far` from the one command. The agent prints the report's table into the note: the endings, the reach, the phase spans, the drop ledger by line, take-by-slot, offers banked while one stood, and the belch fires inside the boss spans. This is where the report stops being a design and becomes a measurement.
 ...
 * person and for a scripted wander (ADR 0027, the record's section 5).
 */
...
**`holdBound` is a bound and not a range, because the low end is always zero.** A hold is drawn from 0 to the bound inclusive, so steady is 0, loose is 6 and shaky is 12, at `TICK_HZ` 60 (`clock.ts:4`). Writing a two-ended range would put a second number in every row that is zero in all nine.
...
**The header this module writes** is the harness rig's: the run's own seed, resolved starting size and levels and roster off `createRun`'s defaults (`run.ts:250-254`), `TICK_HZ`, `RECORDER_CHECKPOINT_SPACING`, `WITNESS_VERSION`, the passed commit hash, an empty build identity, `unknown` as the author, `inputDevice: 'bot'` and `policy: configuration.name`. It is the second copy of a headless header literal in the tree beside `record-conditioned.ts:139-158`; the rule of three says a second copy is fine, and **the third copy is the trigger to extract one**, which is recorded here so the next writer sees it rather than discovering it.
...
export type { BatchIdentity, BatchReport, Spread, UnverifiedRun };
```
...
**`READINGS_VERSION` does not move** (`readingsVersion.ts:28`). Its own rule is that adding a reading beside unchanged ones does not bump it, and every existing figure the drop ledger prints means exactly what it meant.

### `src/dev/replayTallies.ts` and `src/dev/measure.ts` (changed)

`ReplayTallies` gains `shotsAlivePerTick: number[]`, seeded `[0]` beside `mobsAlivePerTick` (`replayTallies.ts:34`, `:110`) and pushed from `liveCount(state.mobFire)` beside it (`replayTallies.ts:153`). `Metrics` gains `shotsAlivePerTick` beside `mobsAlivePerTick` (`measure.ts:117`, `:282`). It is #39's airborne-projectile figure, and it is here because the existing shot count is sampled only where a frame row exists (section 0) and a headless tape has none.
...
`tapeHeaderFor` (`tapeHeader.ts:70-92`) writes it, beside `UNNAMED_AUTHOR` (`:18`) and `UNRESOLVED_BUILD` (`:21`), which is the pattern the reserved fields already use. `record-conditioned.ts`'s `headerFor` (`:139-158`) writes its own `SCRIPT_POLICY = 'script'`, because a fixed arithmetic wander (`:165-170`) is not a policy and is not a person, and its device already says `script`.
...
```
pnpm vite-node --config vite.headless.config.ts scripts/batch.ts <configuration> <first-seed> [count]
```

The shell, and the only thing in this step that touches the filesystem. It refuses an unknown configuration name, a seed outside `SEED_LIMIT` (`run.ts:156`) and a count below one, each out loud with the usage, in the shape `record-conditioned.ts:43-47` already uses: a flawed argument is an external failure and the person holding the command line is the nearest owner who can act. It asks git for the commit hash the way `record-conditioned.ts:126-132` does. Then, per seed: play through `playHarnessRun`, write `local/batches/<configuration>-<stamp>/<seed>.tape`, decode and measure those same bytes, and collect the measurement. Finally it writes `report.json` beside the tapes and prints the folder path as the whole of stdout.
...
| `src/dev/readings/readings.ts` | Changed | Registers `offerChoices` in the four places the graph is declared (`readings.ts:78-92`, `:94-109`, `:117-133`, `:144-159`, `:166-181`). | This dispatch |
| `src/dev/replayTallies.ts` | Changed | `shotsAlivePerTick` beside `mobsAlivePerTick`, which is #39's airborne figure made readable off a headless tape. | This dispatch |
| `src/dev/measure.ts` | Changed | `shotsAlivePerTick` on `Metrics`; the policy on `Provenance`; the policy exclusion beside the device's. | This dispatch |
...
| `src/tape/tape.ts`, `records.ts`, `segments.ts`, `wireCodes.ts` | Changed | The `policy` field in the positional header, and `FORMAT_VERSION` 3. | This dispatch |
| `src/app/tapeHeader.ts` | Changed | `PERSON_POLICY`, written at the one site that builds a person's header. The only file under `src/app` this step touches. | This dispatch |
| `scripts/record-conditioned.ts` | Changed | `SCRIPT_POLICY` in its own header literal. Its steering is untouched. | This dispatch |
...
| `CONTEXT.md` | Changed | Six glossary entries, each landing in the same commit as the word enters the code: Configuration, Rig, Sharp hand and sloppy hand with slice 1; Batch with slice 4b; Dexterity error and Strategy error with slice 5. | This dispatch |
...
33. *The overlap search covers every stream a run holds and the hand's beside them.* Pins ADR 0012 and closes #113: the test's own comment says it is written so that adding a fifth colliding stream name fails loudly (`rng.test.ts:33-35`), and the fifth was added and the list was not.
...
51. The stream is made from the run's seed and the hand's own name, so two configurations on one seed hold different sequences and one configuration on one seed holds the same one twice.
...
65. Shots alive per tick starts at the empty field and follows the shot pool, index by index, the way the mob series does.
...
79. Every new reading on a verified report declares what comparing it means. The walk reaches every leaf (`:69-85`), so `offerChoices`, the widened drop ledger and `shotsAlivePerTick` each need an entry in `READING_COMPARISONS` or this goes red.
...
**Counts: 45 spec tests, 30 module tests, 7 fence and cross-cutting guards. 82 in all.**
...
**`READINGS_VERSION` (`readingsVersion.ts:28`): does not move.**
...
**The bump is taken once and carries one field, because the director's budget is build-pinned.** Record section 5 and section 12's second commitment: the budget is authored stage content, a column on the phase, and no URL pins it and no seed draws it, so ADR 0027 does not reach it. The trigger that reopens it is named: the day the budget becomes something a run resolves.
...
- **The airborne-projectile figure is not readable off a headless tape today.** The record's section 4 lists it among #39's four instruments the report carries. Live shots are counted only inside `densityOf` (`replayTallies.ts:74-80`), which is sampled at expensive-frame ticks (`:158`), and a headless tape has no frame rows. This plan adds `shotsAlivePerTick` beside `mobsAlivePerTick` to close it; without that the report would have carried three of the four and said it carried four.
...
**Slice 1. The hand.** `bot.ts`'s look-ahead becomes a parameter, `LOOKAHEAD_TICKS` retires, and `bestMoveToward`, `nearestFood` and `LOOKAHEAD_SAMPLES` are exported. `configurations.ts` lands with **one row, `steady-far`**, plus `SHARP_HAND`, `RESERVED_POLICIES` and the parse. `harnessPolicy.ts` lands with the three-clause wanting rule and the belch, and with no hold and no stream. Spec tests 1 to 9, module tests 46 to 48, 52, 54 and 75, and fences 76 to 78 land here. `CONTEXT.md` gains Configuration, Rig, and Sharp hand and sloppy hand. **ADR 0053 is amended in place here**, with the belch, because this is the slice that builds it.
...
**Slice 2. The two event fields and the readings.** `OfferOpened.site` and `OfferTaken.slot` in `events.ts`, written at `standOffer`'s two callers and at `resolveOffer`; `offerChoices.ts` registered in the readings graph; `dropLedger` widened by line; `shotsAlivePerTick` in `replayTallies.ts` and on `Metrics`; the `READING_COMPARISONS` entries that keep `comparisonDeclared.test.ts` green. Spec tests 19 to 26, module tests 65, 72 to 74, and guard 79 land here. **`GOLDEN` must not move and `READINGS_VERSION` must not move**; if either does, the slice is wrong.

**Slice 3. The header field, and the one bump.** `TapeHeader.policy`, the write and the read in the positional header, `FORMAT_VERSION` 3, `PERSON_POLICY` in `tapeHeader.ts`, `SCRIPT_POLICY` in `record-conditioned.ts`, the policy on `Provenance` and in `exclusionsOf`, and every one of the fourteen header literals in section 7. Spec tests 27 to 32, module tests 66 and 69, land here. **Verification step 7, the old-tape decode check, runs here on slice 0's two tapes and its result goes in the note.** **ADR 0056 is amended in place here**, because this is the slice that spends the version and the amendment is what says why it spends it once.
...
**Slice 4a. The runner.** `harnessRun.ts` with the derived tick budget and the harness rig's header; `scripts/batch.ts` writing one tape per seed under `steady-far` and no report at all. Spec test 45, module tests 55 to 58, 70 and 71 land here. The agent records a small batch, measures two of its tapes with `scripts/measure.ts` by hand, and puts both readings in the note: this is the first proof that a harness tape replays and attests as a person's does.

**Slice 4b. The report.** `fiveNumbersOf` in `seriesSummary.ts`; `batchReport.ts` with `BATCH_READINGS` and `batchReportOf`; `scripts/batch.ts` gains `report.json`; guards 80 and 81. Spec tests 36 to 41, module tests 59 to 64, land here. `CONTEXT.md` gains Batch. **Verification step 10, the first 48-seed batch under the sharp corner, runs here and its table goes in the note.** **A ends here**, and at this point the harness plays, records, and reports, under one hand.
...
**Slice 5. The two knobs.** The other eight configuration rows; the hold and the `hand` stream in `harnessPolicy.ts`; `stream`'s name parameter widened; `rng.test.ts`'s `NAMES` closed over every member of `StreamName` plus `hand`, which closes #113. Spec tests 10 to 16 and 33 to 35, module tests 49 to 51 and 53, and guard 82 land here. `CONTEXT.md` gains Dexterity error and Strategy error. **Verification step 9, the determinism run under `shaky-short`, runs here.** **B begins here.**
