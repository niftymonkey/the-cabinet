 ...
 * kind and a byte length in front of its payload. A reader can skip a chunk kind
 * it does not understand, which is what "separable sections" has to mean if it
 * is to mean anything.
 */
 ...
 *
 * Nothing in the format needs a value known only at run end: a section is
 * written as as many chunks as the writer feels like, so a store that appends
 * during a run never has to go back and fill in a count. That is why a body
 * chunk carries the tick it starts at and no tick count. The count is the
 * payload's own size, and a recording that stops mid-stream simply ends after
 * the last whole command it managed to write.
 */
const writeChunk = (
  writer: ByteWriter,
  kind: number,
