 ...
 */
const apcaLuminance = (hex: number): number => {
  const [r, g, b] = channels(hex);
  return (
...
const darkOnLight = (textY: number, backgroundY: number): number => {
...
const lightOnDark = (textY: number, backgroundY: number): number => {
...
const apcaLc = (foreground: number, background: number): number => {
...
const hueOf = (r: number, g: number, b: number, max: number, span: number) => {
...
export { luma, apcaLc, observerLuma, hsv };
export type { Observer, Hsv };
